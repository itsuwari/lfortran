...
static bool is_c_rank1_matmul_expr(ASR::expr_t *x) {
    x = ASRUtils::get_past_array_physical_cast(x);
    if (x == nullptr || !ASR::is_a<ASR::IntrinsicArrayFunction_t>(*x)) {
        return false;
    }
    ASR::IntrinsicArrayFunction_t *matmul =
        ASR::down_cast<ASR::IntrinsicArrayFunction_t>(x);
    ...
    if (result_type == nullptr || result_element_type == nullptr
            ...
            || !(ASRUtils::is_real(*result_element_type)
                || ASRUtils::is_integer(*result_element_type)
                || ASRUtils::is_unsigned_integer(*result_element_type))) {
        return false;
    }
...
static bool is_c_rank2_matmul_expr(ASR::expr_t *x) {
    x = ASRUtils::get_past_array_physical_cast(x);
    if (x == nullptr || !ASR::is_a<ASR::IntrinsicArrayFunction_t>(*x)) {
        return false;
    }
...
static bool c_expr_contains_rank1_matmul_expr(ASR::expr_t *x) {
    x = ASRUtils::get_past_array_physical_cast(x);
    if (x == nullptr) {
        return false;
    }
    switch (x->type) {
        case ASR::exprType::IntrinsicArrayFunction: {
            if (is_c_rank1_matmul_expr(x)) {
                return true;
            }
            ASR::IntrinsicArrayFunction_t *f =
                ASR::down_cast<ASR::IntrinsicArrayFunction_t>(x);
            for (size_t i = 0; i < f->n_args; i++) {
                if (c_expr_contains_rank1_matmul_expr(f->m_args[i])) {
                    return true;
                }
            }
            return false;
        ...
        case ASR::exprType::ArrayBroadcast: {
            return c_expr_contains_rank1_matmul_expr(
                ASR::down_cast<ASR::ArrayBroadcast_t>(x)->m_array);
        }
        case ASR::exprType::RealBinOp: {
            ASR::RealBinOp_t *b = ASR::down_cast<ASR::RealBinOp_t>(x);
            return c_expr_contains_rank1_matmul_expr(b->m_left)
                || c_expr_contains_rank1_matmul_expr(b->m_right);
        }
        case ASR::exprType::IntegerBinOp: {
        ...
        case ASR::exprType::UnsignedIntegerBinOp: {
            ASR::UnsignedIntegerBinOp_t *b =
                ASR::down_cast<ASR::UnsignedIntegerBinOp_t>(x);
            return c_expr_contains_rank1_matmul_expr(b->m_left)
                || c_expr_contains_rank1_matmul_expr(b->m_right);
        }
        case ASR::exprType::RealUnaryMinus: {
            return c_expr_contains_rank1_matmul_expr(
                ASR::down_cast<ASR::RealUnaryMinus_t>(x)->m_arg);
        }
        case ASR::exprType::IntegerUnaryMinus: {
            return c_expr_contains_rank1_matmul_expr(
                ASR::down_cast<ASR::IntegerUnaryMinus_t>(x)->m_arg);
        }
        case ASR::exprType::IntrinsicElementalFunction: {
...
static bool is_c_rank1_matmul_elementwise_expr(ASR::expr_t *x) {
    x = ASRUtils::get_past_array_physical_cast(x);
    return x != nullptr && is_c_elementwise_array_expr(x)
        && c_expr_contains_rank1_matmul_expr(x);
}
...
static bool is_c_elementwise_array_expr(ASR::expr_t* x) {
    ...
    switch (x->type) {
        ...
        case ASR::exprType::IntrinsicArrayFunction: {
            return is_c_rank1_matmul_expr(x);
        }
        default: {
            return false;
        }
...
static ASR::ttype_t* normalize_string_temporary_type(
    ...
    ASR::String_t* str_type = ASR::down_cast<ASR::String_t>(
        ASRUtils::extract_type(type_duplicated));
    if (str_type->m_len_kind == ASR::string_length_kindType::AssumedLength) {
...
bool set_allocation_size(
    ...
    switch( value->type ) {
        ...
        case ASR::exprType::ArrayItem: {
            ...
            if( ASRUtils::is_character(*ASRUtils::expr_type(value)) ) {
                ASRUtils::ASRBuilder b(al, loc);
                len_allocte_expr = b.StringLen(array_item_t->m_v);
            }
            break;
        }
        case ASR::exprType::Var: {
...
bool insert_allocate_stmt_for_array(Allocator& al, ASR::expr_t* temporary_var,
...
static inline void push_unique_array_temp_cleanup(Allocator& al,
    Vec<ASR::expr_t*>* cleanup_temps, ASR::expr_t* temporary_var) {
    if (cleanup_temps == nullptr || temporary_var == nullptr) {
        return;
    }
    if (!ASRUtils::is_allocatable(ASRUtils::expr_type(temporary_var))) {
        return;
    }
    for (size_t i = 0; i < cleanup_temps->size(); i++) {
        if ((*cleanup_temps)[i] == temporary_var) {
            return;
        }
    }
    cleanup_temps->push_back(al, temporary_var);
}

static inline void append_array_temp_cleanups(Allocator& al,
    Vec<ASR::stmt_t*>* current_body, Vec<ASR::expr_t*>& cleanup_temps) {
    for (size_t i = 0; i < cleanup_temps.size(); i++) {
        Vec<ASR::expr_t*> dealloc_args;
        dealloc_args.reserve(al, 1);
        dealloc_args.push_back(al, cleanup_temps[i]);
        current_body->push_back(al, ASRUtils::STMT(ASR::make_ImplicitDeallocate_t(
            al, cleanup_temps[i]->base.loc, dealloc_args.p, dealloc_args.size())));
    }
    cleanup_temps.n = 0;
}

static inline bool should_defer_temp_allocation_to_array_op(ASR::expr_t* array_expr) {
    array_expr = ASRUtils::get_past_array_physical_cast(array_expr);
    if (!ASRUtils::is_array(ASRUtils::expr_type(array_expr))) {
        return false;
    }
    if (ASR::is_a<ASR::Var_t>(*array_expr) ||
            ASR::is_a<ASR::ArraySection_t>(*array_expr) ||
            ASR::is_a<ASR::ArrayItem_t>(*array_expr) ||
            ASR::is_a<ASR::ArrayConstant_t>(*array_expr) ||
            ASR::is_a<ASR::ArrayConstructor_t>(*array_expr) ||
            ASR::is_a<ASR::ImpliedDoLoop_t>(*array_expr)) {
        return false;
    }
    return is_vectorise_able(array_expr);
}
    ...
    if( ASRUtils::is_pointer(ASRUtils::expr_type(array_var_temporary)) ) {
        ...
        if (!defer_allocation_to_array_op) {
            allocated_temporary = insert_allocate_stmt_for_array(
                al, array_var_temporary, allocate_size_reference, current_body);
        }
        if (allocated_temporary) {
            push_unique_array_temp_cleanup(al, cleanup_temps, array_var_temporary);
        } else if (defer_allocation_to_array_op) {
            push_unique_array_temp_cleanup(al, cleanup_temps, array_var_temporary);
        }
...
static bool is_explicit_shape_array_type(ASR::ttype_t *type) {
...
static bool is_sequence_association_array_section(ASR::ArraySection_t *section) {
    ASR::ttype_t* source_type = ASRUtils::expr_type(section->m_v);
...
static bool is_c_external_sequence_association_callee(bool c_backend,
        ASR::Function_t *func) {
    (void)c_backend;
    if (func == nullptr) {
        return false;
    ...
    return func_type->m_abi == ASR::abiType::BindC
        && func_type->m_bindc_name == nullptr;
}

static bool should_preserve_c_sequence_association_actual(bool c_backend,
        ASR::Function_t *func, ASR::expr_t *dummy, ASR::expr_t *actual) {
    if (!is_c_external_sequence_association_callee(c_backend, func)
            || dummy == nullptr || actual == nullptr
            || !ASRUtils::is_array(ASRUtils::expr_type(dummy))) {
        return false;
    }
    ASR::expr_t *actual_unwrapped = ASRUtils::get_past_array_physical_cast(actual);
    if (!ASR::is_a<ASR::ArraySection_t>(*actual_unwrapped)) {
        return false;
    }
    return is_sequence_association_array_section(
...
static bool is_c_projected_array_section_expr(ASR::expr_t *expr) {
    ...
    size_t slice_dims = 0;
    size_t scalar_dims = 0;
    for (size_t i = 0; i < section->n_args; i++) {
        if (is_default_unit_array_slice(section->m_args[i])) {
            slice_dims++;
        } else if (is_scalar_array_index(section->m_args[i])) {
            scalar_dims++;
        } else {
            return false;
        }
...
static bool c_projected_ref_has_same_base_and_shape(
    ...
    ASR::ArraySection_t *expr_section = ASR::down_cast<ASR::ArraySection_t>(expr);
    if (!same_array_reference(target_section->m_v, expr_section->m_v)
            || target_section->n_args != expr_section->n_args) {
        return false;
    }
    for (size_t i = 0; i < target_section->n_args; i++) {
        bool target_slice = is_default_unit_array_slice(target_section->m_args[i]);
        bool expr_slice = is_default_unit_array_slice(expr_section->m_args[i]);
        bool target_scalar = is_scalar_array_index(target_section->m_args[i]);
        bool expr_scalar = is_scalar_array_index(expr_section->m_args[i]);
        if ((target_slice && expr_slice) || (target_scalar && expr_scalar)) {
            continue;
        }
        return false;
    }
...
static bool rhs_refs_are_c_projected_same_base(Allocator &al,
    ...
    bool saw_common_ref = false;
    for (size_t i = 0; i < rhs_vars.size(); i++) {
        if (!ASRUtils::is_array(ASRUtils::expr_type(rhs_vars[i]))) {
            continue;
        }
        if (extract_symbol(rhs_vars[i]) != lhs_sym
...
static bool is_c_vector_subscript_expr(ASR::expr_t *expr) {
    if (expr == nullptr) {
        return false;
    }
    expr = ASRUtils::get_past_array_physical_cast(expr);
...
static bool is_c_struct_member_vector_subscript_array_expr(ASR::expr_t *expr) {
    expr = ASRUtils::get_past_array_physical_cast(expr);
    ...
    ASR::ttype_t *element_type = expr_type != nullptr
        ? ASRUtils::type_get_past_array(expr_type) : nullptr;
...
static bool is_c_pointer_array_base_expr(ASR::expr_t *expr) {
    expr = ASRUtils::get_past_array_physical_cast(expr);
    if (expr == nullptr) {
        return false;
    }
...
static bool is_c_plain_scalar_array_element_type(ASR::ttype_t *type) {
    if (type == nullptr) {
        return false;
    }
...
static bool is_c_rank2_scalarizable_reshape(ASR::expr_t *expr) {
    expr = ASRUtils::get_past_array_physical_cast(expr);
...
static bool is_c_proven_contiguous_no_copy_base(ASR::expr_t *base) {
    base = ASRUtils::get_past_array_physical_cast(base);
    ASR::Variable_t *var = ASRUtils::expr_to_variable_or_null(base);
    if (var == nullptr || ASRUtils::is_pointer(var->m_type)) {
        return false;
    }
    ASR::ttype_t *type = ASRUtils::type_get_past_allocatable_pointer(
        var->m_type);
    ...
    return var->m_contiguous_attr
        || ASRUtils::is_allocatable(var->m_type)
        || ASRUtils::is_fixed_size_array(var->m_type);
}

static bool is_c_packed_contiguous_array_section_view(
        ASR::ArraySection_t *section) {
    if (section == nullptr) {
        return false;
    }
    ...
    ASR::ttype_t *base_type = base != nullptr
        ? ASRUtils::type_get_past_allocatable_pointer(ASRUtils::expr_type(base))
        : nullptr;
    ASR::ttype_t *section_type = ASRUtils::type_get_past_allocatable_pointer(
        section->m_type);
    int base_rank = ASRUtils::extract_n_dims_from_ttype(base_type);
    int section_rank = ASRUtils::extract_n_dims_from_ttype(section_type);
    if (base_rank <= 0 || section_rank != base_rank
            || section->n_args != static_cast<size_t>(base_rank)
            || !is_c_proven_contiguous_no_copy_base(base)) {
        return false;
    }
    for (int i = 0; i < base_rank; i++) {
        const ASR::array_index_t &idx = section->m_args[i];
        ...
        if (i + 1 < base_rank && !is_default_unit_array_slice(idx)) {
            return false;
        }
    }
    return true;
...
static bool should_preserve_c_no_copy_section_actual(bool c_backend,
    ...
    if (dummy_requires_contiguous
            && !is_c_packed_contiguous_array_section_view(section)) {
        return false;
    }
    ASR::expr_t *base = ASRUtils::get_past_array_physical_cast(section->m_v);
    if (base == nullptr
...
static ASR::Variable_t *get_c_raw_helper_arg_var(ASR::Function_t *func,
        size_t i) {
...
static bool is_c_raw_helper_scalar_arg(const ASR::Variable_t *arg) {
    ASR::ttype_t *arg_type_unwrapped = arg == nullptr ? nullptr
        : ASRUtils::type_get_past_allocatable_pointer(arg->m_type);
    if (arg == nullptr || arg_type_unwrapped == nullptr
    ...
    return is_c_plain_scalar_array_element_type(arg_type_unwrapped);
}

static bool is_c_raw_helper_array_arg(const ASR::Variable_t *arg) {
    if (arg == nullptr || !ASRUtils::is_arg_dummy(arg->m_intent)
            || arg->m_presence == ASR::presenceType::Optional
            || arg->m_value_attr || arg->m_target_attr
            || ASRUtils::is_allocatable(arg->m_type)
            || ASRUtils::is_pointer(arg->m_type)) {
...
static bool is_c_private_or_internal_source_subroutine(
        ASR::Function_t *func) {
    if (func == nullptr || std::string(func->m_name) == "main") {
        return false;
    }
    ASR::FunctionType_t *func_type = ASRUtils::get_FunctionType(func);
    if (func_type == nullptr || func_type->m_abi != ASR::abiType::Source
...
static ASR::expr_t *get_c_raw_helper_array_index_expr(
        const ASR::array_index_t &idx) {
...
static bool is_c_raw_helper_raw_var_expr(ASR::expr_t *expr,
...
class CBackendRawArrayUseVerifier:
        public ASR::BaseWalkVisitor<CBackendRawArrayUseVerifier> {
public:
    const std::set<ASR::symbol_t*> &raw_arg_symbols;
    bool ok = true;

    CBackendRawArrayUseVerifier(
            const std::set<ASR::symbol_t*> &raw_arg_symbols):
        raw_arg_symbols(raw_arg_symbols) {}

    bool is_raw_var(ASR::expr_t *expr) {
        return is_c_raw_helper_raw_var_expr(expr, raw_arg_symbols);
    }
    ...
    void visit_ArrayItem(const ASR::ArrayItem_t &x) {
        ...
        if (!is_raw_var(x.m_v)) {
            ASR::BaseWalkVisitor<CBackendRawArrayUseVerifier>::
                visit_ArrayItem(x);
            return;
        }
        if (ASRUtils::is_array(x.m_type)) {
        ...
        for (size_t i = 0; i < x.n_args; i++) {
            ASR::expr_t *idx_expr =
                get_c_raw_helper_array_index_expr(x.m_args[i]);
            if (idx_expr == nullptr
    ...
    void visit_ArraySection(const ASR::ArraySection_t &x) {
        ...
        for (size_t i = 0; i < x.n_args; i++) {
            ...
            if (idx.m_left != nullptr) {
                if (ASRUtils::is_array(ASRUtils::expr_type(idx.m_left))) {
                    ok = false;
                    return;
                }
            ...
            if (idx.m_right != nullptr) {
                if (ASRUtils::is_array(ASRUtils::expr_type(idx.m_right))) {
                    ok = false;
                    return;
                }
            ...
            if (idx.m_step != nullptr) {
                if (ASRUtils::is_array(ASRUtils::expr_type(idx.m_step))) {
                    ok = false;
                    return;
                }
...
static bool can_specialize_c_raw_helper_body(ASR::Function_t *func,
    ...
    CBackendRawArrayUseVerifier verifier(raw_arg_symbols);
...
static bool get_c_raw_helper_args(bool c_backend, ASR::Function_t *func,
        std::vector<size_t> &raw_positions) {
    raw_positions.clear();
    if (!c_backend || !is_c_private_or_internal_source_subroutine(func)) {
        return false;
    }
...
static bool is_c_raw_helper_array_actual(ASR::Variable_t *formal,
        ASR::expr_t *actual) {
    ...
    for (int i = 0; i < base_rank; i++) {
        if (!is_unit_step_expr(indices[i].m_step)
                || !is_c_simple_integer_bound_expr(indices[i].m_left)
                || !is_c_simple_integer_bound_expr(indices[i].m_right)) {
            return false;
        }
    }
    return true;
...
static std::vector<bool> get_c_raw_helper_call_temp_bypass(bool c_backend,
    ...
    if (func == nullptr || n_args != func->n_args
            || !get_c_raw_helper_args(c_backend, func, raw_positions)) {
        return bypass;
    }
    for (size_t pos: raw_positions) {
...
static bool is_c_rank2_allocatable_self_section_assignment(
        ASR::expr_t *lhs_array_var, ASR::expr_t *target, ASR::expr_t *value) {
    target = ASRUtils::get_past_array_physical_cast(target);
    value = ASRUtils::get_past_array_physical_cast(value);
    if (lhs_array_var == nullptr || target == nullptr || value == nullptr
    ...
    if (!ASRUtils::is_array(target_type) || !ASRUtils::is_array(value_type)
            ...
            || ASRUtils::extract_n_dims_from_ttype(value_type) != 2
            || !is_c_plain_scalar_array_element_type(target_type)
            || !is_c_plain_scalar_array_element_type(value_type)) {
        return false;
    }
    ASR::ArraySection_t *section = ASR::down_cast<ASR::ArraySection_t>(value);
    if (section->n_args != 2
            || !same_array_reference(lhs_array_var, section->m_v)) {
        return false;
    }
    for (size_t i = 0; i < section->n_args; i++) {
        const ASR::array_index_t &idx = section->m_args[i];
...
static bool is_c_rank1_allocatable_self_section_assignment(
        ASR::expr_t *lhs_array_var, ASR::expr_t *target, ASR::expr_t *value) {
    target = ASRUtils::get_past_array_physical_cast(target);
    value = ASRUtils::get_past_array_physical_cast(value);
    if (lhs_array_var == nullptr || target == nullptr || value == nullptr
    ...
    bool is_slice = idx.m_left || idx.m_step || idx.m_right == nullptr;
    return is_slice
        && is_unit_step_expr(idx.m_step)
        && is_c_simple_integer_bound_expr(idx.m_left)
        && is_c_simple_integer_bound_expr(idx.m_right);
}

static bool is_c_rank2_nonself_section_copy_assignment(Allocator &al,
        ASR::expr_t *lhs_array_var, ASR::expr_t *target, ASR::expr_t *value) {
    target = ASRUtils::get_past_array_physical_cast(target);
    value = ASRUtils::get_past_array_physical_cast(value);
    if (lhs_array_var == nullptr || target == nullptr || value == nullptr
            ...
            || !ASR::is_a<ASR::ArraySection_t>(*value)
            || !is_c_rank2_unit_slice_array_expr(value)
            || is_common_symbol_present_in_lhs_and_rhs(al, lhs_array_var, value)) {
        return false;
    }
    ASR::ttype_t *target_type = ASRUtils::type_get_past_allocatable_pointer(
        ASRUtils::expr_type(target));
    ASR::ttype_t *value_type = ASRUtils::type_get_past_allocatable_pointer(
        ASRUtils::expr_type(value));
    return ASRUtils::is_array(target_type)
        && ASRUtils::is_array(value_type)
        && is_c_plain_scalar_array_element_type(target_type)
        && is_c_plain_scalar_array_element_type(value_type);
}

static bool is_c_rank1_nonself_section_copy_assignment(Allocator &al,
        ASR::expr_t *lhs_array_var, ASR::expr_t *target, ASR::expr_t *value) {
    target = ASRUtils::get_past_array_physical_cast(target);
    value = ASRUtils::get_past_array_physical_cast(value);
    if (lhs_array_var == nullptr || target == nullptr || value == nullptr
            ...
            || is_c_pointer_array_base_expr(target)
            || is_c_pointer_array_base_expr(value)
            || !is_c_rank1_unit_slice_array_expr(target)
            || !ASR::is_a<ASR::ArraySection_t>(*value)
            || !is_c_rank1_unit_slice_array_expr(value)
            || is_common_symbol_present_in_lhs_and_rhs(al, lhs_array_var, value)) {
        return false;
    }
    ASR::ttype_t *target_type = ASRUtils::type_get_past_allocatable_pointer(
...
static bool is_c_plain_scalar_expr_for_section_fill(ASR::expr_t *expr) {
    expr = ASRUtils::get_past_array_physical_cast(expr);
    if (expr == nullptr) {
        return false;
    }
...
static bool is_c_rank2_spread_scalarizable_expr(
    ...
    return source_type != nullptr
        ...
        && ASRUtils::types_equal(source_element_type, result_element_type,
            nullptr, nullptr)
        && is_c_rank1_scalarizable_array_expr(x.m_args[0]);
}

static bool is_c_rank2_transpose_scalarizable_expr(
        const ASR::IntrinsicArrayFunction_t &x) {
    if (x.m_arr_intrinsic_id != static_cast<int64_t>(
...
static bool is_c_rank2_scalarizable_after_rhs_temps_expr(Allocator &al,
    ...
    switch (expr->type) {
        case ASR::exprType::ArrayReshape: {
            return should_preserve_c_rank2_scalarizable_reshape(
                al, lhs_array_var, expr);
        }
        case ASR::exprType::ArrayBroadcast: {
            return is_c_rank2_scalarizable_after_rhs_temps_expr(
                al, lhs_array_var,
                ASR::down_cast<ASR::ArrayBroadcast_t>(expr)->m_array);
        }
        case ASR::exprType::RealBinOp: {
            ASR::RealBinOp_t *binop = ASR::down_cast<ASR::RealBinOp_t>(expr);
            return binop->m_op != ASR::binopType::Pow
                && is_c_rank2_scalarizable_after_rhs_temps_expr(
                    al, lhs_array_var, binop->m_left)
                && is_c_rank2_scalarizable_after_rhs_temps_expr(
                    al, lhs_array_var, binop->m_right);
        }
        case ASR::exprType::IntegerBinOp: {
            ASR::IntegerBinOp_t *binop = ASR::down_cast<ASR::IntegerBinOp_t>(expr);
            return binop->m_op != ASR::binopType::Pow
...
static bool is_c_rank2_scalarized_nonself_assignment(Allocator &al,
        ASR::expr_t *lhs_array_var, ASR::expr_t *target, ASR::expr_t *value) {
    target = ASRUtils::get_past_array_physical_cast(target);
    value = ASRUtils::get_past_array_physical_cast(value);
    if (lhs_array_var == nullptr || target == nullptr || value == nullptr
    ...
    ASR::ttype_t *value_type = ASRUtils::type_get_past_allocatable_pointer(
        ASRUtils::expr_type(value));
    ASR::ttype_t *target_element_type = target_type != nullptr
        ? ASRUtils::type_get_past_array(target_type) : nullptr;
    ASR::ttype_t *value_element_type = value_type != nullptr
...
static bool contains_c_rank2_scalarizable_reshape(ASR::expr_t *expr) {
    expr = ASRUtils::get_past_array_physical_cast(expr);
    if (expr == nullptr) {
        return false;
    }
    switch (expr->type) {
        ...
        case ASR::exprType::ArrayBroadcast: {
            return contains_c_rank2_scalarizable_reshape(
                ASR::down_cast<ASR::ArrayBroadcast_t>(expr)->m_array);
        }
        case ASR::exprType::RealBinOp: {
            ASR::RealBinOp_t *binop = ASR::down_cast<ASR::RealBinOp_t>(expr);
            return contains_c_rank2_scalarizable_reshape(
                    binop->m_left)
                || contains_c_rank2_scalarizable_reshape(
                    binop->m_right);
        }
        case ASR::exprType::IntegerBinOp: {
            ASR::IntegerBinOp_t *binop = ASR::down_cast<ASR::IntegerBinOp_t>(expr);
...
static bool is_c_rank2_scalarized_reshape_assignment(Allocator &al,
        ASR::expr_t *lhs_array_var, ASR::expr_t *target, ASR::expr_t *value) {
    return is_c_rank2_scalarized_nonself_assignment(
            al, lhs_array_var, target, value)
        && contains_c_rank2_scalarizable_reshape(value);
}

static bool c_rank2_full_ref_same_target(ASR::expr_t *target, ASR::expr_t *expr) {
    target = ASRUtils::get_past_array_physical_cast(target);
    expr = ASRUtils::get_past_array_physical_cast(expr);
    if (!is_c_rank2_full_array_expr(target)
            || !is_c_rank2_full_array_expr(expr)) {
        return false;
    }
    ASR::symbol_t *target_sym = extract_symbol(target);
    ASR::symbol_t *expr_sym = extract_symbol(expr);
    return target_sym != nullptr && target_sym == expr_sym;
}
...
class ArgSimplifier: public ASR::CallReplacerOnExpressionsVisitor<ArgSimplifier>
{
    ...
    bool c_elementwise_self_assignment;
    bool c_elementwise_projected_self_assignment;
    ...
    ArgSimplifier(Allocator& al_, ExprsWithTargetType& exprs_with_target_,
        ...
        al(al_), current_body(nullptr), parent_body_for_where(nullptr),
            ...
            c_elementwise_self_assignment(false),
            c_elementwise_projected_self_assignment(false) {
    ...
    void transform_stmts(ASR::stmt_t**& m_body, size_t& n_body) {
        ...
        current_body = &current_body_vec;
        for (size_t i = 0; i < n_body; i++) {
            statement_cleanup_temps.n = 0;
            statement_cleanup_temps.reserve(al, 1);
            visit_stmt(*m_body[i]);
            current_body->push_back(al, m_body[i]);
            append_array_temp_cleanups(al, current_body, statement_cleanup_temps);
        }
        m_body = current_body_vec.p; n_body = current_body_vec.size();
        current_body = current_body_copy;
    }
    ...
    ASR::expr_t* call_create_and_allocate_temporary_variable(ASR::expr_t*& expr, Allocator &al, Vec<ASR::stmt_t*>*& current_body,
        ...
        array_var_temporary = create_and_allocate_temporary_variable_for_array(
            ...
            is_pointer_required, nullptr, false, &statement_cleanup_temps);
        return array_var_temporary;
    }
    ...
    void traverse_call_args(Vec<ASR::call_arg_t>& x_m_args_vec, ASR::call_arg_t* x_m_args,
        ...
        std::vector<bool> bypass_raw_helper_array_temps =
            get_c_raw_helper_call_temp_bypass(
                c_backend, func, x_m_args, x_n_args);
        for( size_t i = 0; i < x_n_args; i++ ) {
            ...
            if (orig_args && func && i < func->n_args &&
                    should_preserve_c_sequence_association_actual(
                        c_backend, func, orig_args[i], x_m_args[i].m_value)) {
                x_m_args_vec.push_back(al, x_m_args[i]);
                continue;
            }
            if (orig_args && func && i < func->n_args &&
    ...
    void visit_Assignment(const ASR::Assignment_t& x) {
        ...
        bool saved_c_elementwise_self_assignment =
            c_elementwise_self_assignment;
        bool saved_c_elementwise_projected_self_assignment =
        ...
        bool c_rank1_matmul_scalarized_assignment = c_backend
            && lhs_array_var != nullptr
            && ASRUtils::is_array(ASRUtils::expr_type(x.m_value))
            && is_c_rank1_matmul_elementwise_expr(x.m_value)
            && (!is_common_symbol_present_in_lhs_and_rhs(al, lhs_array_var, x.m_value)
                || rhs_common_refs_match_lhs_ref(al, x.m_target, x.m_value)
                || rhs_refs_are_c_projected_same_base(al, x.m_target, x.m_value));
        bool c_rank2_allocatable_matmul_assignment = c_backend
            && lhs_array_var != nullptr
            && ASRUtils::is_allocatable(x.m_target)
            && ASRUtils::is_array(ASRUtils::expr_type(x.m_value))
            && is_c_rank2_matmul_expr(x.m_value)
            && !is_common_symbol_present_in_lhs_and_rhs(al, lhs_array_var, x.m_value);
        c_elementwise_self_assignment = (c_backend
            && lhs_array_var != nullptr
            && ASRUtils::is_array(ASRUtils::expr_type(x.m_value))
            && is_c_elementwise_array_expr(x.m_value)
            && rhs_common_refs_match_lhs_ref(al, x.m_target, x.m_value))
            || c_rank1_matmul_scalarized_assignment;
        c_elementwise_projected_self_assignment = c_backend
            && lhs_array_var != nullptr
            && ASRUtils::is_array(ASRUtils::expr_type(x.m_value))
            && is_c_elementwise_array_expr(x.m_value)
            && rhs_refs_are_c_projected_same_base(al, x.m_target, x.m_value);
        bool c_rank2_scalarized_reshape_assignment = c_backend &&
            is_c_rank2_scalarized_reshape_assignment(
                al, lhs_array_var, x.m_target, x.m_value);
        lhs_var = lhs_array_var;
        if (!c_rank2_scalarized_reshape_assignment
                && !c_rank2_allocatable_matmul_assignment) {
        ...
        c_elementwise_self_assignment =
            saved_c_elementwise_self_assignment;
        c_elementwise_projected_self_assignment =
            saved_c_elementwise_projected_self_assignment;
    }
    ...
    ASR::expr_t* visit_BinOp_expr(ASR::expr_t* expr, const std::string& name_hint, ASR::exprType allowed_expr) {
        ...
        if (c_backend
                && (c_elementwise_self_assignment
                    || c_elementwise_nonalias_vector_assignment
                    || c_elementwise_projected_self_assignment)
                && is_c_rank1_matmul_elementwise_expr(expr)) {
            return expr;
        }
        if (c_backend
                && should_preserve_c_rank2_scalarizable_reshape(
                    al, lhs_var, expr)) {
            return expr;
        }
    ...
    void visit_Call(const T& x, const std::string& name_hint,
        ...
        if (ASR::is_a<ASR::Function_t>(*sym)) {
            func = ASR::down_cast<ASR::Function_t>(sym);
    ...
    void visit_StructConstructor(const ASR::StructConstructor_t& x) {
        ...
        Vec<ASR::call_arg_t> x_m_args; x_m_args.reserve(al, x.n_args);
        traverse_call_args(x_m_args, x.m_args, x.n_args, nullptr, nullptr,
    ...
    void visit_BitCast(const ASR::BitCast_t& x) {
        // transfer(source, mold, ...) must not evaluate or copy the mold; it
        // only supplies result shape/type metadata. Copying an intent(out)
        // mold section reads undefined data and can crash.
...
class ReplaceExprWithTemporary: public ASR::BaseExprReplacer<ReplaceExprWithTemporary> {
    ...
    bool is_current_expr_linked_to_target_then_return(ASR::expr_t** &current_expr, ExprsWithTargetType& exprs_with_target,
        ...
        if( is_current_expr_linked_to_target(exprs_with_target, current_expr) ) {
            ...
            if( realloc_target != target &&
                is_whole_allocatable_target &&
                ASRUtils::is_allocatable(ASRUtils::expr_type(realloc_target)) &&
                target_Type == targetType::OriginalTarget &&
                realloc_lhs ) {
                insert_allocate_stmt_for_array(al, realloc_target, *current_expr,
                                               current_body, target);
            } else if( is_whole_allocatable_target &&
    ...
    void replace_IntrinsicArrayFunction(ASR::IntrinsicArrayFunction_t* x) {
        ...
        bool lhs_is_whole_allocatable =
            lhs_expr_unwrapped != nullptr
            && !ASR::is_a<ASR::ArraySection_t>(*lhs_expr_unwrapped)
            && !ASR::is_a<ASR::ArrayItem_t>(*lhs_expr_unwrapped)
            && ASRUtils::is_allocatable(lhs_expr);
        if (c_backend
                && lhs_expr != nullptr
                && !lhs_is_whole_allocatable
                && is_c_rank1_matmul_expr(*current_expr)) {
            return;
        }
        if (c_backend
                && lhs_expr != nullptr
                && !is_c_compiler_created_array_temp_expr(lhs_expr)
                && is_c_rank2_full_array_expr(lhs_expr)
                && is_c_rank2_scalarizable_array_expr(*current_expr)) {
            return;
        }
    ...
    void replace_BitCast(ASR::BitCast_t* x) {
        ...
        replace_expr(x->m_source);
        current_expr = current_expr_copy;
        if (x->m_size) {
            current_expr_copy = current_expr;
            current_expr = &(x->m_size);
            replace_expr(x->m_size);
            current_expr = current_expr_copy;
        }
    }
...
class ReplaceExprWithTemporaryVisitor:
    ...
    void call_replacer() {
        ...
        replacer.statement_cleanup_temps = &statement_cleanup_temps;
    ...
    ASR::Function_t* get_call_function(ASR::symbol_t *name) {
        ...
        if (ASR::is_a<ASR::StructMethodDeclaration_t>(*sym)) {
            ...
            ASR::symbol_t* proc = ASRUtils::symbol_get_past_external(smd->m_proc);
            if (ASR::is_a<ASR::Function_t>(*proc)) {
                return ASR::down_cast<ASR::Function_t>(proc);
            }
        }
        return nullptr;
    }
    ...
    bool should_skip_call_arg_replacement(ASR::Function_t *func,
        ...
        std::vector<bool> bypass_raw_helper_array_temps =
            get_c_raw_helper_call_temp_bypass(c_backend, func, args, n_args);
        if (bypass_raw_helper_array_temps.size() > i
                && bypass_raw_helper_array_temps[i]) {
            return true;
        }
    ...
    void visit_Assignment(const ASR::Assignment_t &x) {
        ...
        ASR::expr_t* original_lhs_expr = x.m_target;
        ASR::array_index_t* m_args = nullptr; size_t n_args = 0;
        ASR::expr_t* lhs_array_var = nullptr;
        if( ASRUtils::is_array(ASRUtils::expr_type(x.m_target)) ) {
            lhs_array_var = ASRUtils::extract_array_variable(x.m_target);
        ...
        bool c_rank2_section_scalar_assignment = c_backend &&
            is_c_rank2_section_scalar_assignment(x.m_target, x.m_value);
        ...
        if( !c_rank2_section_scalar_assignment &&
            ...
            !c_rank2_scalarized_reshape_assignment &&
        ...
        replacer.lhs_expr = original_lhs_expr;
        bool c_rank1_matmul_scalarized_assignment = c_backend
            && lhs_array_var != nullptr
            && ASRUtils::is_array(ASRUtils::expr_type(x.m_value))
            && is_c_rank1_matmul_elementwise_expr(x.m_value)
            && (!is_common_symbol_present_in_lhs_and_rhs(al, lhs_array_var, x.m_value)
                || rhs_common_refs_match_lhs_ref(al, original_lhs_expr, x.m_value)
        ...
        bool c_elementwise_self_assignment = (c_backend
            ...
            && rhs_common_refs_match_lhs_ref(al, original_lhs_expr, x.m_value))
            || c_rank1_matmul_scalarized_assignment;
        bool c_elementwise_projected_self_assignment = c_backend
            && lhs_array_var != nullptr
            && ASRUtils::is_array(ASRUtils::expr_type(x.m_value))
            && is_c_elementwise_array_expr(x.m_value)
            && rhs_refs_are_c_projected_same_base(al, original_lhs_expr, x.m_value);
        ...
        bool is_self_ref_allocatable_array = lhs_array_var &&
            ...
            is_common_symbol_present_in_lhs_and_rhs(al, lhs_array_var, x.m_value);
        bool c_rank2_allocatable_self_section_assignment = c_backend &&
            is_self_ref_allocatable_array &&
            is_c_rank2_allocatable_self_section_assignment(
                lhs_array_var, x.m_target, x.m_value);
        bool c_rank1_allocatable_self_section_assignment = c_backend &&
            is_self_ref_allocatable_array &&
            is_c_rank1_allocatable_self_section_assignment(
                lhs_array_var, x.m_target, x.m_value);
        bool c_rank2_nonself_section_copy_assignment = c_backend &&
            is_c_rank2_nonself_section_copy_assignment(
                al, lhs_array_var, x.m_target, x.m_value);
        ...
        bool saved_c_elementwise_self_assignment =
            replacer.c_elementwise_self_assignment;
        bool saved_c_elementwise_projected_self_assignment =
            replacer.c_elementwise_projected_self_assignment;
        replacer.c_elementwise_self_assignment =
            c_elementwise_self_assignment;
        replacer.c_elementwise_projected_self_assignment =
            c_elementwise_projected_self_assignment;
        ...
        if (!c_rank2_allocatable_self_section_assignment
        ...
        if(  (!c_rank2_scalarized_reshape_assignment &&
            !c_rank2_allocatable_matmul_assignment &&
            is_assignment_target_array_section_item && !c_elementwise_nonalias_vector_target) ||
...
class VerifySimplifierASROutput:
    ...
    VerifySimplifierASROutput(Allocator& al_, ExprsWithTargetType& exprs_with_target_,
            bool c_backend_) :
        al(al_), exprs_with_target(exprs_with_target_), c_backend(c_backend_) {
    ...
    void check_for_var_if_array(ASR::expr_t* expr) {
        if ( is_temporary_needed(expr) ) {
            if (c_backend && is_c_rank2_scalarizable_array_expr(expr)) {
                return;
            }
    ...
    void traverse_call_args(ASR::call_arg_t* m_args, size_t n_args,
        ...
        for( size_t i = 0; i < n_args; i++ ) {
            ...
            if (orig_args && func && i < func->n_args &&
                    should_preserve_c_sequence_association_actual(
                        c_backend, func, orig_args[i], m_args[i].m_value)) {
                continue;
            }
            if (orig_args && func && i < func->n_args &&
                    should_preserve_c_no_copy_section_actual(
...
void pass_array_struct_temporary(Allocator &al, ASR::TranslationUnit_t &unit,
    ...
    VerifySimplifierASROutput e(al, exprs_with_target, pass_options.c_backend);
