...
public:
    ...
    std::vector<std::string> current_function_local_stack_character_strings;
    ...
    std::string emit_current_function_heap_array_cleanup(const std::string &indent,
        ...
        for (auto it = current_function_local_stack_character_strings.rbegin();
                it != current_function_local_stack_character_strings.rend(); ++it) {
            cleanup += indent + "_lfortran_string_len_unregister(" + *it + ");\n";
        }
    ...
    void clear_current_function_cleanup_state() {
        ...
        current_function_local_stack_character_strings.clear();
    ...
    void register_current_function_local_stack_character_string_cleanup(
            const std::string &target) {
        for (const std::string &existing: current_function_local_stack_character_strings) {
            if (existing == target) {
                return;
            }
        }
        current_function_local_stack_character_strings.push_back(target);
    }
    ...
    void register_c_local_variable_cleanup(const ASR::Variable_t &v,
        ...
        if (ASRUtils::is_allocatable(v.m_type)
            ...
            ASR::ttype_t *v_type_unwrapped =
                ASRUtils::type_get_past_allocatable_pointer(v.m_type);
            if (ASRUtils::is_character(*v_type_unwrapped)) {
                if (CUtils::use_stack_storage_for_fixed_character_scalar(v)) {
                    register_current_function_local_stack_character_string_cleanup(
                        emitted_name);
                } else {
...
#include <lfortran_intrinsics.h>
    ...
    std::string get_emitted_function_name(const ASR::Function_t &x) {
        ...
        if (x.n_args > 0) {
            ASR::Variable_t *arg0 = nullptr;
            if (x.m_args[0] != nullptr && ASR::is_a<ASR::Var_t>(*x.m_args[0])) {
                ASR::symbol_t *arg0_sym = ASRUtils::symbol_get_past_external(
                    ASR::down_cast<ASR::Var_t>(x.m_args[0])->m_v);
                if (arg0_sym != nullptr && ASR::is_a<ASR::Variable_t>(*arg0_sym)) {
                    arg0 = ASR::down_cast<ASR::Variable_t>(arg0_sym);
                }
            }
            if (arg0 != nullptr) {
                ASR::ttype_t *arg0_type = ASRUtils::type_get_past_allocatable_pointer(arg0->m_type);
                ASR::ttype_t *arg0_base = arg0_type != nullptr
                    ? ASRUtils::extract_type(arg0_type) : nullptr;
                if ((arg0_base != nullptr && ASR::is_a<ASR::StructType_t>(*arg0_base))
                        || (arg0_type != nullptr && ASRUtils::is_class_type(arg0_type))) {
    ...
    bool is_global_source_implementation_function(const ASR::Function_t &x) {
        ASR::FunctionType_t *f_type = ASRUtils::get_FunctionType(x);
        if (f_type->m_abi != ASR::abiType::Source
                || f_type->m_deftype != ASR::deftypeType::Implementation
                || CUtils::is_translation_unit_local_helper_symbol(
                    reinterpret_cast<const ASR::symbol_t*>(&x))
                || std::string(x.m_name) == "main") {
            return false;
        }
        ...
        return parent != nullptr
            && !(parent->asr_owner != nullptr
                && ASR::is_a<ASR::symbol_t>(*parent->asr_owner));
    }

    std::string get_fortran_external_wrapper_arg_decl(
            const ASR::Variable_t &arg, bool &has_typevar,
            std::string &public_name) {
        std::string arg_name = CUtils::sanitize_c_identifier(arg.m_name);
        public_name = arg_name;
        ASR::ttype_t *arg_type =
            ASRUtils::type_get_past_allocatable_pointer(arg.m_type);
        if (arg_type == nullptr || ASR::is_a<ASR::TypeParameter_t>(*arg_type)) {
            has_typevar = true;
            return "";
        }
        ...
        std::string type_name = get_fortran_external_value_type(arg_type);
        if (expects_c_scalar_ref_actual(&arg)) {
            return format_type_c("", type_name + " *", arg_name, false, true);
        }
        public_name = "__lfortran_f77_" + arg_name + "_ptr";
        return format_type_c("", type_name + " *", public_name, false, true);
    }

    std::string get_fortran_external_wrapper_dim_expr(ASR::expr_t *expr,
            const std::string &fallback) {
        if (expr == nullptr) {
            return fallback;
        }
    ...
    std::string get_fortran_external_abi_wrapper(const ASR::Function_t &x) {
        if (!is_c || !compiler_options.implicit_interface
                || !is_global_source_implementation_function(x)) {
            return "";
        }
        bool has_typevar = false;
        std::string wrapper_name = CUtils::sanitize_c_identifier(x.m_name);
        if (wrapper_name.empty()
                || wrapper_name == get_emitted_function_name(x)) {
            return "";
        }

        std::string return_type = "void ";
        if (x.m_return_var) {
            return_type = get_return_var_type(ASRUtils::EXPR2VAR(x.m_return_var));
        }
        std::string wrapper = return_type + wrapper_name + "(";
        std::string scalar_setup;
        std::string descriptor_setup;
        ...
        for (size_t i = 0; i < x.n_args; i++) {
            ...
            if (ASR::is_a<ASR::Variable_t>(*sym)) {
                ...
                std::string public_name;
                std::string arg_decl =
                    get_fortran_external_wrapper_arg_decl(*arg, has_typevar, public_name);
                if (has_typevar || arg_decl.empty()) {
                    return "";
                }
                if (i > 0) {
                    wrapper += ", ";
                ...
                if (ASRUtils::is_array(arg->m_type)) {
                    ...
                    std::string wrapper_type =
                        get_c_array_wrapper_type_name(arg->m_type, arg->m_type_declaration);
                    std::string desc_name = "__lfortran_f77_" + arg_name + "_desc";
                    std::string dim_entries;
                    std::string stride = "1";
                    for (int d = 0; d < n_dims; d++) {
                        std::string lower =
                            get_fortran_external_wrapper_dim_expr(dims[d].m_start, "1");
                        std::string length =
                            get_fortran_external_wrapper_dim_expr(dims[d].m_length, "1");
                        if (d > 0) {
                            dim_entries += ", ";
                        }
                        dim_entries += "{" + lower + ", " + length + ", " + stride + "}";
                    ...
                    arg_call = arg_name;
                } else if (expects_c_scalar_ref_actual(arg)) {
                    arg_call = arg_name;
                } else {
                    std::string type_name = get_fortran_external_value_type(arg->m_type);
                    scalar_setup += indent + type_name + " " + arg_name + " = *"
                        + public_name + ";\n";
                    arg_call = arg_name;
                }
            } else if (ASR::is_a<ASR::Function_t>(*sym)) {
                ...
                wrapper += get_function_declaration(*fun, has_typevar, true, false);
                if (has_typevar) {
                    return "";
                }
                arg_call = get_c_function_target_name(*fun);
            } else {
                return "";
            }
            call_args += arg_call;
        ...
        wrapper += ")\n{\n";
        wrapper += scalar_setup;
        wrapper += descriptor_setup;
        if (x.m_return_var) {
            wrapper += indent + "return " + get_emitted_function_name(x)
                + "(" + call_args + ");\n";
        } else {
            wrapper += indent + get_emitted_function_name(x)
                + "(" + call_args + ");\n";
        }
        wrapper += "}\n";
        return wrapper;
    ...
    bool is_c_unspecified_scalar_ref_dummy(const ASR::Variable_t *v) {
        if (!is_c || v == nullptr || v->m_intent != ASRUtils::intent_unspecified
                || v->m_value_attr || !ASRUtils::is_arg_dummy(v->m_intent)
                || ASRUtils::is_array(v->m_type)
                || ASRUtils::is_allocatable(v->m_type)
                || ASRUtils::is_pointer(v->m_type)
                || ASRUtils::is_aggregate_type(v->m_type)) {
        ...
        ASR::ttype_t *type =
            ASRUtils::type_get_past_allocatable_pointer(v->m_type);
        if (type == nullptr || ASRUtils::is_character(*type)
                || ASR::is_a<ASR::FunctionType_t>(*type)
                || ASR::is_a<ASR::CPtr_t>(*type)) {
            return false;
        }
    ...
    bool expects_c_scalar_ref_actual(const ASR::Variable_t *v) {
        ASR::ttype_t *type = v == nullptr ? nullptr
            : ASRUtils::type_get_past_allocatable_pointer(v->m_type);
        if (!is_c || v == nullptr || !ASRUtils::is_arg_dummy(v->m_intent)
                || ASRUtils::is_array(v->m_type)
                || type == nullptr || ASRUtils::is_character(*type)) {
            return false;
        }
    ...
    bool is_fortran_external_interface_function(const ASR::Function_t &x) {
        ...
        ASR::FunctionType_t *f_type = ASRUtils::get_FunctionType(x);
        if (f_type->m_abi == ASR::abiType::Source
                && is_procedure_dummy_function_argument(x)) {
            return false;
        }
        ...
        if (f_type->m_abi == ASR::abiType::BindC) {
            if (!compiler_options.implicit_interface || f_type->m_bindc_name) {
                return false;
            }
        } else if (f_type->m_abi != ASR::abiType::Source) {
            return false;
        }
        if (x.n_args == 0) {
            return true;
    ...
    bool is_procedure_dummy_function_argument(const ASR::Function_t &x) {
        if (x.m_symtab == nullptr || x.m_symtab->parent == nullptr
        ...
        if (owner_sym == nullptr || !ASR::is_a<ASR::Function_t>(*owner_sym)) {
            return false;
        }
        ASR::Function_t *owner_fn = ASR::down_cast<ASR::Function_t>(owner_sym);
        uint64_t target_hash = get_hash(reinterpret_cast<ASR::asr_t*>(
            const_cast<ASR::Function_t*>(&x)));
        for (size_t i=0; i<owner_fn->n_args; i++) {
            ...
            if (arg_sym == nullptr || !ASR::is_a<ASR::Function_t>(*arg_sym)) {
                continue;
            }
            uint64_t arg_hash = get_hash(reinterpret_cast<ASR::asr_t*>(arg_sym));
            if (arg_hash == target_hash) {
                return true;
            }
        }
        return false;
    ...
    void visit_Function(const ASR::Function_t &x) {
        ...
        std::string fortran_external_abi_wrapper =
            get_fortran_external_abi_wrapper(x);
        if (!fortran_external_abi_wrapper.empty()) {
            sub += fortran_external_abi_wrapper + "\n";
        }
...
PyMODINIT_FUNC PyInit_lpython_module_)" + fn_name + R"((void) {
    ...
    bool is_pointer_backed_struct_expr(ASR::expr_t *expr) {
        ...
        if (ASR::is_a<ASR::Var_t>(*expr)) {
            ...
            ASR::asr_t *owner = v->m_parent_symtab ? v->m_parent_symtab->asr_owner : nullptr;
            bool common_block_struct_instance = owner
                && ASR::is_a<ASR::Module_t>(*owner)
                && std::string(ASR::down_cast<ASR::Module_t>(owner)->m_name)
                    .find("file_common_block_") == 0
                && std::string(v->m_name).find("struct_instance_") == 0;
            return ASRUtils::is_arg_dummy(v->m_intent)
                || ASRUtils::is_pointer(v->m_type)
                || ASRUtils::is_allocatable(v->m_type)
                || emitted_pointer_backed_struct
                || common_block_struct_instance
                || (owner
    ...
    bool get_c_rank1_simple_section_access(ASR::expr_t *expr,
        ...
        ASR::dimension_t *base_dims = nullptr;
        int base_rank = ASRUtils::extract_dimensions_from_ttype(base_type, base_dims);
        if (base_rank != 1 || section->n_args != 1) {
            return false;
        }
        ...
        int64_t static_lb = 1;
        if (use_static_base_dims) {
            if (base_dims[0].m_start != nullptr
                    && !get_c_constant_int_expr_value(base_dims[0].m_start, static_lb)) {
                use_static_base_dims = false;
            }
        }

        std::string base_lb = use_static_base_dims
        ...
        std::string indent(indentation_level * indentation_spaces, ' ');
        ASR::ttype_t *element_type = ASRUtils::type_get_past_array(base_type);
        std::string elem_type = CUtils::get_c_type_from_ttype_t(element_type);
        data_name = get_unique_local_name(prefix + "_data");
        offset_name = get_unique_local_name(prefix + "_offset");
        stride_name = get_unique_local_name(prefix + "_stride");
    ...
    bool try_emit_c_rank1_section_array_constant_assignment(
            const ASR::Assignment_t &x, ASR::expr_t *target_expr) {
        if (!is_c) {
            return false;
        }
        target_expr = unwrap_c_lvalue_expr(target_expr);
        if (target_expr == nullptr || !ASR::is_a<ASR::ArraySection_t>(*target_expr)) {
            return false;
        }
        ASR::ArrayConstant_t *arr = get_c_array_constant_expr(x.m_value);
        ASR::ArrayConstructor_t *ctor = nullptr;
        ASR::expr_t *value_expr = unwrap_c_array_expr(x.m_value);
        if (arr == nullptr && value_expr != nullptr
                && ASR::is_a<ASR::ArrayConstructor_t>(*value_expr)) {
            ctor = ASR::down_cast<ASR::ArrayConstructor_t>(value_expr);
            if (!is_c_scalarizable_array_constructor(*ctor)) {
                return false;
            }
        }
        if (arr == nullptr && ctor == nullptr) {
            return false;
        }
        ASR::ttype_t *raw_value_type = arr ? arr->m_type : ctor->m_type;
        ...
        ASR::ttype_t *element_type = ASRUtils::type_get_past_array(value_type);
        if (element_type == nullptr
                || !is_c_scalarizable_element_type(element_type)
                || ASRUtils::is_character(*element_type)) {
            return false;
        }
        ...
        if (target_element_type == nullptr
                || !ASRUtils::check_equal_type(target_element_type, element_type,
                    nullptr, nullptr)) {
            return false;
        }
        ...
        if (ctor != nullptr && static_cast<int64_t>(ctor->n_args) != length) {
            return false;
        }
        ...
        if (arr != nullptr) {
            for (int64_t i = 0; i < length; i++) {
                values.push_back(get_c_array_constant_init_element_for_c_index(
                    arr, value_type, element_type, static_cast<size_t>(i)));
            }
        } else {
            for (int64_t i = 0; i < length; i++) {
                self().visit_expr(*ctor->m_args[i]);
    ...
    bool is_c_simple_integer_section_bound_expr(ASR::expr_t *expr) {
        ...
        switch (expr->type) {
            ...
            case ASR::exprType::IntegerUnaryMinus: {
                return is_c_simple_integer_section_bound_expr(
                    ASR::down_cast<ASR::IntegerUnaryMinus_t>(expr)->m_arg);
            }
            case ASR::exprType::IntegerBinOp: {
                ASR::IntegerBinOp_t *binop = ASR::down_cast<ASR::IntegerBinOp_t>(expr);
                return is_c_simple_integer_section_bound_expr(binop->m_left)
                    && is_c_simple_integer_section_bound_expr(binop->m_right);
            }
            default: {
                return false;
            }
    ...
    bool is_c_rank2_unit_slice_array_expr(ASR::expr_t *expr) {
        expr = unwrap_c_array_expr(expr);
        if (expr == nullptr || (!ASRUtils::is_array(ASRUtils::expr_type(expr))
                && !ASRUtils::is_array_t(expr))) {
            return false;
        ...
        ASR::ttype_t *type = ASRUtils::type_get_past_allocatable_pointer(
            ASRUtils::expr_type(expr));
        int n_dims = ASRUtils::extract_n_dims_from_ttype(ASRUtils::expr_type(expr));
        bool scalarizable_element = type != nullptr && is_c_scalarizable_element_type(type);
        if (type == nullptr
                || n_dims != 2
                || !scalarizable_element) {
            return false;
        }
        if (ASR::is_a<ASR::Var_t>(*expr)
                || ASR::is_a<ASR::StructInstanceMember_t>(*expr)) {
    ...
    bool is_c_plain_scalar_expr_for_section_fill(ASR::expr_t *expr) {
        expr = unwrap_c_array_expr(expr);
        if (expr == nullptr) {
            return false;
        }
    ...
    bool is_c_reallocated_temp_plain_copy_context(
            ASR::expr_t *target_expr, ASR::expr_t *value_expr) {
        return is_c_compiler_created_array_temp_expr(target_expr)
            || is_c_compiler_created_array_temp_expr(value_expr)
            || is_c_array_section_association_temp_expr(value_expr);
    }
    ...
    bool try_emit_c_scalarized_array_expr_assignment(
        ...
        ASR::ttype_t *target_type = ASRUtils::type_get_past_allocatable_pointer(
            ASRUtils::expr_type(plan.target_expr));
        ASR::ttype_t *target_element_type = target_type
            ? ASRUtils::type_get_past_array(target_type) : nullptr;
        ASR::ttype_t *value_type = ASRUtils::type_get_past_allocatable_pointer(
        ...
        if (target_element_type != nullptr
                ...
                && ASRUtils::is_character(*value_type)) {
            std::string char_target_length = target_length;
            if (char_target_length.empty() && compile_time_length > 0) {
                char_target_length = std::to_string(compile_time_length);
            }
            if (char_target_length.empty()) {
                return false;
            }
            std::string value_expr;
            std::string value_len;
            std::string value_cleanup;
            if (is_allocating_string_expr(plan.value_expr)) {
                materialize_allocating_string_expr(plan.value_expr, value_expr,
                    value_len, setup, value_cleanup);
            } else {
                self().visit_expr(*plan.value_expr);
                value_expr = src;
                setup += drain_tmp_buffer();
                setup += extract_stmt_setup_from_expr(value_expr);
                materialize_allocating_string_expr(plan.value_expr, value_expr,
                    value_len, setup, value_cleanup);
            }
            std::string indent(indentation_level * indentation_spaces, ' ');
    ...
    bool try_emit_c_self_section_realloc_assignment(
            ASR::expr_t *target_expr, ASR::expr_t *value_expr) {
        target_expr = unwrap_c_lvalue_expr(target_expr);
        value_expr = unwrap_c_lvalue_expr(value_expr);
        if (!is_c || target_expr == nullptr || value_expr == nullptr
                || !is_c_whole_allocatable_or_pointer_array_expr(target_expr)
                || !ASRUtils::is_allocatable(ASRUtils::expr_type(target_expr))
                || !ASR::is_a<ASR::ArraySection_t>(*value_expr)) {
            return false;
        }
        ASR::ArraySection_t *source_section =
            ASR::down_cast<ASR::ArraySection_t>(value_expr);
        ASR::symbol_t *target_root = get_c_array_assignment_root_symbol(target_expr);
        ASR::symbol_t *source_root =
            get_c_array_assignment_root_symbol(source_section->m_v);
        if (target_root == nullptr || target_root != source_root) {
            return false;
        }
        ASR::ttype_t *target_type = ASRUtils::type_get_past_allocatable_pointer(
            ASRUtils::expr_type(target_expr));
        ...
        ASR::ttype_t *element_type = ASRUtils::type_get_past_array(target_type);
        ASR::ttype_t *value_element_type = ASRUtils::type_get_past_array(value_type);
        if (element_type == nullptr || value_element_type == nullptr
                || ASRUtils::is_character(*element_type)
                || ASRUtils::is_character(*value_element_type)
                || !is_c_scalarizable_element_type(element_type)
                || !is_c_scalarizable_element_type(value_element_type)
                || CUtils::get_c_type_from_ttype_t(element_type)
                    != CUtils::get_c_type_from_ttype_t(value_element_type)) {
            return false;
        }
        int target_rank = ASRUtils::extract_n_dims_from_ttype(target_type);
        if (target_rank != 1 && target_rank != 2) {
            return false;
        }
        if ((target_rank == 1 && !is_c_rank1_unit_slice_array_expr(value_expr))
                || (target_rank == 2 && !is_c_rank2_unit_slice_array_expr(value_expr))) {
            return false;
        }
        std::string elem_type = CUtils::get_c_type_from_ttype_t(element_type);
        std::string allocator_expr = "_lfortran_get_default_allocator()";
        std::string setup;
        ...
        if (target_rank == 1) {
            ...
            if (!get_c_rank1_array_access(value_expr, "__lfortran_self_rhs1",
                    setup, source_data, source_offset, source_stride,
                    source_length)) {
                return false;
            }
            body = check_tmp_buffer();
            body += indent + "{\n";
            indentation_level++;
            std::string inner_indent(indentation_level * indentation_spaces, ' ');
            std::string old_data = get_unique_local_name(
                "__lfortran_self_realloc_old_data");
            std::string new_data = get_unique_local_name(
                "__lfortran_self_realloc_new_data");
            std::string new_size = get_unique_local_name(
                "__lfortran_self_realloc_new_size");
            std::string index_name = get_unique_local_name("__lfortran_i");
            std::string source_index = get_unique_local_name(
                "__lfortran_self_rhs1_index");

            body += setup;
            body += inner_indent + elem_type + " *" + old_data + " = "
                + target + "->data;\n";
            body += inner_indent + "int64_t " + new_size + " = "
                + source_length + ";\n";
            body += inner_indent + elem_type + " *" + new_data + " = NULL;\n";
            body += inner_indent + "if (" + new_size + " > 0) {\n";
            indentation_level++;
            std::string alloc_indent(indentation_level * indentation_spaces, ' ');
            body += alloc_indent + new_data + " = (" + elem_type
                + "*) _lfortran_malloc_alloc(" + allocator_expr + ", "
                + "(size_t)(" + new_size + ") * sizeof(*" + new_data + "));\n";
            body += alloc_indent + "if (" + source_stride + " == 1) {\n";
            indentation_level++;
            std::string memcpy_indent(indentation_level * indentation_spaces, ' ');
            body += memcpy_indent + "memcpy(" + new_data + ", " + source_data
                + " + " + source_offset + ", (size_t)(" + new_size
                + ") * sizeof(*" + new_data + "));\n";
            indentation_level--;
            body += alloc_indent + "} else {\n";
            indentation_level++;
            std::string loop_indent(indentation_level * indentation_spaces, ' ');
            body += loop_indent + "int64_t " + source_index + " = "
                + source_offset + ";\n";
            body += loop_indent + "for (int64_t " + index_name + " = 0; "
                + index_name + " < " + new_size + "; " + index_name
                + "++) {\n";
            indentation_level++;
            std::string item_indent(indentation_level * indentation_spaces, ' ');
            body += item_indent + new_data + "[" + index_name + "] = "
                + source_data + "[" + source_index + "];\n";
            body += item_indent + source_index + " += " + source_stride + ";\n";
            indentation_level--;
            body += loop_indent + "}\n";
            indentation_level--;
            body += alloc_indent + "}\n";
            indentation_level--;
            body += inner_indent + "}\n";
            body += inner_indent + "if (" + target + "->is_allocated && "
                + old_data + " != NULL) {\n";
            body += inner_indent + "    _lfortran_free_alloc_plain(" + allocator_expr + ", (char*) "
                + old_data + ");\n";
            body += inner_indent + "}\n";
            body += inner_indent + target + "->data = " + new_data + ";\n";
            ...
            std::string source_data, source_offset, source_stride1, source_stride2;
            std::string source_length1, source_length2;
            if (!get_c_rank2_full_array_access(value_expr, "__lfortran_self_rhs2",
                    setup, source_data, source_offset, source_stride1,
                    source_stride2, source_length1, source_length2)) {
                return false;
            }
            body = check_tmp_buffer();
            body += indent + "{\n";
            ...
            std::string new_size = get_unique_local_name(
                "__lfortran_self_realloc_new_size");
            std::string col_index = get_unique_local_name("__lfortran_j");
            std::string row_index = get_unique_local_name("__lfortran_i");
            std::string source_col = get_unique_local_name(
                "__lfortran_self_rhs2_col");
            std::string source_item = get_unique_local_name(
                "__lfortran_self_rhs2_item");
            std::string target_col = get_unique_local_name(
                "__lfortran_self_lhs2_col");
            ...
            body += alloc_indent + new_data + " = (" + elem_type
                + "*) _lfortran_malloc_alloc(" + allocator_expr + ", "
                + "(size_t)(" + new_size + ") * sizeof(*" + new_data + "));\n";
            body += alloc_indent + "if (" + source_stride1 + " == 1) {\n";
            indentation_level++;
            std::string memcpy_indent(indentation_level * indentation_spaces, ' ');
            body += memcpy_indent + "for (int64_t " + col_index + " = 0, "
                + source_col + " = " + source_offset + ", " + target_col
                + " = 0; " + col_index + " < " + source_length2 + "; "
                + col_index + "++, " + source_col + " += " + source_stride2
                + ", " + target_col + " += " + source_length1 + ") {\n";
            indentation_level++;
            std::string memcpy_loop_indent(indentation_level * indentation_spaces, ' ');
            body += memcpy_loop_indent + "memcpy(" + new_data + " + "
                + target_col + ", " + source_data + " + " + source_col
                + ", (size_t)(" + source_length1 + ") * sizeof(*"
                + new_data + "));\n";
            indentation_level--;
            body += memcpy_indent + "}\n";
            indentation_level--;
            ...
            indentation_level++;
            std::string loop_indent(indentation_level * indentation_spaces, ' ');
            body += loop_indent + "for (int64_t " + col_index + " = 0, "
                + source_col + " = " + source_offset + ", " + target_col
                + " = 0; " + col_index + " < " + source_length2 + "; "
                + col_index + "++, " + source_col + " += " + source_stride2
                + ", " + target_col + " += " + source_length1 + ") {\n";
            indentation_level++;
            std::string col_indent(indentation_level * indentation_spaces, ' ');
            body += col_indent + "int64_t " + source_item + " = "
                + source_col + ";\n";
            body += col_indent + "for (int64_t " + row_index + " = 0; "
                + row_index + " < " + source_length1 + "; " + row_index
                + "++) {\n";
            indentation_level++;
            std::string item_indent(indentation_level * indentation_spaces, ' ');
            body += item_indent + new_data + "[" + target_col + " + "
                + row_index + "] = " + source_data + "[" + source_item
                + "];\n";
            body += item_indent + source_item + " += " + source_stride1 + ";\n";
            indentation_level--;
            body += col_indent + "}\n";
            indentation_level--;
            body += loop_indent + "}\n";
            ...
            body += inner_indent + target + "->dims[0].stride = 1;\n";
            body += inner_indent + target + "->dims[1].lower_bound = 1;\n";
            body += inner_indent + target + "->dims[1].length = " + source_length2 + ";\n";
            body += inner_indent + target + "->dims[1].stride = " + source_length1 + ";\n";
        }
        indentation_level--;
        body += indent + "}\n";
        src = body;
    ...
    bool try_emit_c_rank2_plain_data_copy_assignment(
            ASR::expr_t *target_expr, ASR::expr_t *value_expr) {
        target_expr = unwrap_c_lvalue_expr(target_expr);
        value_expr = unwrap_c_lvalue_expr(value_expr);
        ...
        bool target_is_whole_allocatable =
            ...
            && ASRUtils::is_allocatable(ASRUtils::expr_type(target_expr));
        bool target_is_reallocated_temp_copy =
            target_is_whole_allocatable
            && is_c_reallocated_temp_plain_copy_context(target_expr, value_expr);
        bool value_is_pointer_base = value_expr != nullptr
            && is_c_pointer_array_base_expr(value_expr);
        bool target_is_rank2_unit =
            target_expr != nullptr && is_c_rank2_unit_slice_array_expr(target_expr);
        bool value_is_plain_copy =
            value_expr != nullptr && is_c_rank2_plain_data_copy_expr(value_expr);
        if (!is_c || target_expr == nullptr || value_expr == nullptr
                || (target_is_whole_alloc_pointer
                    && !target_is_reallocated_temp_copy)
        ...
        if (!get_c_rank2_full_array_access(target_expr, "__lfortran_lhs2",
        ...
        std::string inner_indent(indentation_level * indentation_spaces, ' ');
        src += setup;
        src += inner_indent + "if (" + target_stride1 + " == 1 && "
            + source_stride1 + " == 1) {\n";
        indentation_level++;
        std::string memcpy_indent(indentation_level * indentation_spaces, ' ');
        ...
        src += row_indent + "int64_t " + source_index + " = " + source_row + ";\n";
        ...
        src += item_indent + source_index + " += " + source_stride1 + ";\n";
        indentation_level--;
        src += row_indent + "}\n";
        indentation_level--;
        src += loop_indent + "}\n";
    ...
    bool try_emit_c_rank1_plain_data_copy_assignment(
        ...
        if (direct_memcpy_only) {
            ...
            indentation_level++;
            std::string loop_indent(indentation_level * indentation_spaces, ' ');
            std::string target_index = get_unique_local_name("__lfortran_lhs1_index");
            std::string source_index = get_unique_local_name("__lfortran_rhs1_index");
            src += loop_indent + "int64_t " + target_index + " = "
                + target_offset + ";\n";
            src += loop_indent + "int64_t " + source_index + " = "
                + source_offset + ";\n";
            ...
            src += item_indent + target_data + "[" + target_index + "] = "
                + source_data + "[" + source_index + "];\n";
            src += item_indent + target_index + " += " + target_stride + ";\n";
            src += item_indent + source_index + " += " + source_stride + ";\n";
            indentation_level--;
            src += loop_indent + "}\n";
            indentation_level--;
            src += inner_indent + "}\n";
    ...
    bool try_emit_c_rank2_full_self_update_assignment(
        ...
        if (plan.kind == CArrayExprLoweringKind::Rank2ScalarizedLoop
                && try_emit_c_rank2_plain_data_copy_assignment(
    ...
    bool try_emit_c_rank2_section_scalar_assignment(
            ASR::expr_t *target_expr, ASR::expr_t *value_expr) {
        target_expr = unwrap_c_lvalue_expr(target_expr);
        if (!is_c || target_expr == nullptr || value_expr == nullptr
                || !ASR::is_a<ASR::ArraySection_t>(*target_expr)
                || !is_c_rank2_unit_slice_array_expr(target_expr)
                || !is_c_plain_scalar_expr_for_section_fill(value_expr)) {
            return false;
        }
        ASR::ttype_t *target_type = ASRUtils::type_get_past_allocatable_pointer(
            ASRUtils::expr_type(target_expr));
        ...
        ASR::ttype_t *value_type = ASRUtils::type_get_past_allocatable_pointer(
            ASRUtils::expr_type(value_expr));
        if (target_element_type == nullptr || value_type == nullptr
                || !ASRUtils::types_equal(
        ...
        std::string target_data, target_offset, target_stride1, target_stride2;
        std::string target_length1, target_length2;
        if (!get_c_rank2_full_array_access(target_expr, "__lfortran_lhs2fill",
                setup, target_data, target_offset, target_stride1, target_stride2,
                target_length1, target_length2)) {
            return false;
        }
        ...
        std::string index2_name = get_unique_local_name("__lfortran_j");
        std::string value;
        if (!get_c_rank2_scalarized_expr(value_expr, index1_name,
                index2_name, setup, value)) {
            return false;
        }
    ...
    bool try_emit_c_reshape_char_array_bitcast_to_numeric_array_assignment(
            const ASR::Assignment_t &x, ASR::expr_t *target_expr) {
        if (!is_c || target_expr == nullptr
        ...
        ASR::expr_t *reshape_array = reshape->m_array;
        ASR::expr_t *reshape_array_value = ASRUtils::expr_value(reshape_array);
        if (!ASR::is_a<ASR::BitCast_t>(*reshape_array)
                && reshape_array_value != nullptr) {
            reshape_array = reshape_array_value;
        }
        if (reshape_array != nullptr && ASR::is_a<ASR::Var_t>(*reshape_array)) {
            ASR::symbol_t *reshape_array_sym = ASRUtils::symbol_get_past_external(
                ASR::down_cast<ASR::Var_t>(reshape_array)->m_v);
            if (reshape_array_sym != nullptr
                    && ASR::is_a<ASR::Variable_t>(*reshape_array_sym)) {
                ASR::Variable_t *reshape_array_var =
                    ASR::down_cast<ASR::Variable_t>(reshape_array_sym);
                if (reshape_array_var->m_symbolic_value != nullptr) {
                    reshape_array = reshape_array_var->m_symbolic_value;
                } else if (reshape_array_var->m_value != nullptr) {
                    reshape_array = reshape_array_var->m_value;
                }
            }
        }
        reshape_array = unwrap_c_array_expr(reshape_array);
        if (reshape->m_pad != nullptr || reshape->m_order != nullptr
                || reshape_array == nullptr
                || !ASR::is_a<ASR::BitCast_t>(*reshape_array)) {
            return false;
        }
        ASR::ttype_t *target_type = ASRUtils::type_get_past_allocatable_pointer(
            ASRUtils::expr_type(target_expr));
        ...
        int target_rank = ASRUtils::extract_n_dims_from_ttype(target_type);
        if (target_elem_type == nullptr || target_rank < 1
                || ASRUtils::is_character(*target_elem_type)) {
            return false;
        }

        ASR::BitCast_t *bitcast = ASR::down_cast<ASR::BitCast_t>(reshape_array);
        ASR::expr_t *source = bitcast->m_source;
        ASR::ttype_t *source_elem_type = source
            ? ASRUtils::type_get_past_array(
                ASRUtils::type_get_past_allocatable_pointer(
                    ASRUtils::expr_type(source))) : nullptr;
        if (source == nullptr || source_elem_type == nullptr
                || !ASRUtils::is_character(*source_elem_type)) {
            return false;
        }
        ...
        setup += drain_tmp_buffer();
        setup += extract_stmt_setup_from_expr(target);
        std::string target_data = get_c_array_data_expr(target_expr, target);
        std::string target_offset = get_c_array_offset_expr(target_expr, target);
        std::string elem_type = CUtils::get_c_type_from_ttype_t(target_elem_type);
        ...
        offset_terms.push_back(target_offset);
        for (int d = 0; d < target_rank; d++) {
            std::string dim = std::to_string(d);
            std::string idx = get_unique_local_name("__lfortran_transfer_i" + dim);
            idx_names.push_back(idx);
            offset_terms.push_back(target + "->dims[" + dim + "].stride * ("
                + idx + " - " + target + "->dims[" + dim + "].lower_bound)");
        }
        ...
        for (int d = target_rank - 1; d >= 0; d--) {
            ...
            src += inner_indent + "for (int64_t " + idx_names[d] + " = "
                + target + "->dims[" + dim + "].lower_bound; " + idx_names[d]
                + " <= " + target + "->dims[" + dim + "].lower_bound + "
                + target + "->dims[" + dim + "].length - 1; "
                + idx_names[d] + "++) {\n";
            indentation_level++;
        ...
        for (size_t i = 0; i < offset_terms.size(); i++) {
            ...
            target_index += offset_terms[i];
        }
        target_index += ")";
        src += inner_indent + "unsigned char *" + dst_bytes + " = (unsigned char*) &"
            + target_data + "[" + target_index + "];\n";
        src += inner_indent + "memset(" + dst_bytes + ", 0, sizeof(" + elem_type + "));\n";
        src += inner_indent + "for (int64_t " + byte_idx + " = 0; "
            + byte_idx + " < (int64_t) sizeof(" + elem_type + ") && "
            + src_idx + " < " + source_length + "; " + byte_idx + "++, "
            + src_idx + "++) {\n";
        indentation_level++;
    ...
    std::string get_c_array_wrapper_data_pointer_type(ASR::ttype_t *type,
            const std::string &wrapper_type, ASR::symbol_t *type_decl=nullptr) {
        ASR::ttype_t *array_base = get_c_array_wrapper_base_type(type);
        if (array_base == nullptr) {
            return "";
        }
        ...
        std::string element_c_type =
            get_c_concrete_type_from_ttype_t(element_type, element_type_decl);
        if (element_c_type == "void*" && element_type != nullptr
                && ASR::is_a<ASR::StructType_t>(*element_type)
                && wrapper_type.rfind("struct x", 0) == 0) {
            element_c_type = "struct " + wrapper_type.substr(std::string("struct x").size());
        }
    ...
    std::string build_c_array_wrapper_from_cast_target(ASR::ttype_t *target_type,
        ...
        auto build_sequence_array_item_wrapper = [&]() -> std::string {
            ...
            std::string target_data_ptr_type =
    ...
    std::string build_c_array_no_copy_descriptor_view(
        ...
        if (source_section != nullptr) {
            ...
            offset += ")";
            std::string target_data_ptr_type =
                get_c_array_wrapper_data_pointer_type(
                    target_type, target_wrapper, target_type_decl);
        ...
        std::string target_data_ptr_type =
            get_c_array_wrapper_data_pointer_type(target_type, target_wrapper, target_type_decl);
        ...
        if (!use_named_stack_view) {
            std::string view_expr = "(&(" + target_wrapper + "){ .data = (("
                + target_data_ptr_type + ")" + data_ptr
                + "), .dims = {" + dims_init + "}, .n_dims = "
                + std::to_string(target_rank)
                + ", .offset = " + offset
    ...
    bool has_c_value_cast_wrapper(ASR::expr_t *expr) {
        while (expr != nullptr) {
            if (ASR::is_a<ASR::Cast_t>(*expr)
                    || ASR::is_a<ASR::BitCast_t>(*expr)) {
                return true;
            }
            if (ASR::is_a<ASR::GetPointer_t>(*expr)) {
                expr = ASR::down_cast<ASR::GetPointer_t>(expr)->m_arg;
    ...
    bool can_take_fortran_external_scalar_actual_address(
            ASR::expr_t *call_arg, ASR::expr_t *shape_call_arg) {
        if (!is_addressable_fortran_external_actual(shape_call_arg)
                || has_c_value_cast_wrapper(call_arg)) {
            return false;
        }
        ASR::ttype_t *call_type = ASRUtils::type_get_past_allocatable_pointer(
            ASRUtils::expr_type(call_arg));
        ASR::ttype_t *shape_type = ASRUtils::type_get_past_allocatable_pointer(
            ASRUtils::expr_type(shape_call_arg));
        return call_type != nullptr && shape_type != nullptr
            && ASRUtils::check_equal_type(call_type, shape_type, nullptr, nullptr);
    }
    ...
    std::string get_fortran_external_scalar_actual(
        ...
        if (can_take_fortran_external_scalar_actual_address(
                    call_arg, shape_call_arg)) {
    ...
    bool is_scalar_array_item_sequence_actual(ASR::expr_t *expr) {
        expr = unwrap_c_lvalue_expr(expr);
        return expr != nullptr
            && ASR::is_a<ASR::ArrayItem_t>(*expr)
            && !ASRUtils::is_array(ASRUtils::expr_type(expr));
    ...
    bool try_get_fortran_external_array_section_actual(
            ASR::expr_t *shape_call_arg, std::string &out) {
        ASR::expr_t *expr = unwrap_c_lvalue_expr(shape_call_arg);
        if (expr == nullptr || !ASR::is_a<ASR::ArraySection_t>(*expr)) {
            return false;
        }
        ASR::ArraySection_t *section = ASR::down_cast<ASR::ArraySection_t>(expr);
        ASR::expr_t *base_expr = unwrap_c_lvalue_expr(section->m_v);
        if (base_expr == nullptr) {
            return false;
        }
        if (is_data_only_array_expr(base_expr)
                || is_fixed_size_array_storage_expr(base_expr)) {
            return false;
        }
        ASR::ttype_t *base_type = ASRUtils::type_get_past_allocatable_pointer(
        ...
        std::vector<std::string> indices;
        indices.reserve(section->n_args);
        for (size_t i = 0; i < section->n_args; i++) {
            ASR::array_index_t idx = section->m_args[i];
            bool is_slice = idx.m_left || idx.m_step || idx.m_right == nullptr;
            ASR::expr_t *idx_expr = is_slice ? idx.m_left : get_array_index_expr(idx);
            if (idx_expr != nullptr) {
                self().visit_expr(*idx_expr);
                std::string idx_src = src;
                setup += drain_tmp_buffer();
                ...
                indices.push_back(base + "->dims[" + std::to_string(i)
                    + "].lower_bound");
            }
        }
        src = saved_src;
        ...
        std::string index = cmo_convertor_single_element(
            base, indices, base_rank, false);
        out = "(" + get_c_array_data_expr(base_expr, base) + " + " + index + ")";
        return true;
    }

    std::string get_fortran_external_array_actual(
            ASR::expr_t *call_arg, ASR::expr_t *shape_call_arg,
            const std::string &arg_src) {
        if (is_scalar_array_item_sequence_actual(shape_call_arg)) {
            return "&" + get_addressable_call_arg_src(shape_call_arg, arg_src);
        }
        return get_c_array_data_pointer_expr(call_arg, arg_src);
    }

    std::string get_c_procedure_actual_arg(ASR::expr_t *call_arg,
            const std::string &arg_src, bool for_fortran_external_abi) {
        if (call_arg != nullptr && ASR::is_a<ASR::Var_t>(*call_arg)) {
            ASR::symbol_t *actual_sym = ASRUtils::symbol_get_past_external(
                ASR::down_cast<ASR::Var_t>(call_arg)->m_v);
            if (actual_sym != nullptr && ASR::is_a<ASR::Function_t>(*actual_sym)) {
                ASR::Function_t *actual_fn = ASR::down_cast<ASR::Function_t>(actual_sym);
                if (for_fortran_external_abi
                        || is_fortran_external_interface_function(*actual_fn)) {
                    return CUtils::sanitize_c_identifier(actual_fn->m_name);
                }
                return get_c_function_target_name(*actual_fn);
            }
        }
        return arg_src;
    }
    ...
    std::string construct_fortran_external_call_args(ASR::Function_t* f,
        ...
        for (size_t i = start_idx; i < n_args; i++) {
            ...
            ASR::expr_t *raw_call_arg = unwrap_c_lvalue_expr(call_arg);
            ASR::expr_t *shape_call_arg = raw_call_arg ? raw_call_arg : call_arg;
            ASR::Variable_t *param = nullptr;
            ASR::symbol_t *param_sym = nullptr;
            if (i < f->n_args && f->m_args[i] != nullptr
                    && ASR::is_a<ASR::Var_t>(*f->m_args[i])) {
                param_sym = ASRUtils::symbol_get_past_external(
                    ASR::down_cast<ASR::Var_t>(f->m_args[i])->m_v);
            }
            if (param_sym != nullptr && ASR::is_a<ASR::Variable_t>(*param_sym)) {
                param = ASR::down_cast<ASR::Variable_t>(param_sym);
            }
            ASR::ttype_t *param_type = param ? param->m_type : ASRUtils::expr_type(call_arg);
            std::string section_arg;
            if (try_get_fortran_external_array_section_actual(
                    shape_call_arg, section_arg)) {
                args += section_arg;
                if (i < n_args - 1) {
                    args += ", ";
                }
            ...
            if (param_sym != nullptr && ASR::is_a<ASR::Function_t>(*param_sym)) {
                args += get_c_procedure_actual_arg(call_arg, arg_src, true);
                if (i < n_args - 1) {
                    args += ", ";
                }
                continue;
            ...
            if (param_type != nullptr && ASRUtils::is_array(param_type)) {
                args += get_fortran_external_array_actual(
                    call_arg, shape_call_arg, arg_src);
            } else {
    ...
    std::string get_c_scalar_ref_actual(ASR::Variable_t *param,
            ASR::expr_t *call_arg, ASR::expr_t *shape_call_arg,
            const std::string &arg_src) {
        if (can_take_fortran_external_scalar_actual_address(
                    call_arg, shape_call_arg)) {
    ...
    std::string construct_call_args(ASR::Function_t* f, size_t n_args,
        ...
        for (size_t i=0; i<n_args; i++) {
            ...
            if (param_sym != nullptr && ASR::is_a<ASR::Function_t>(*param_sym)) {
                ...
                args += get_c_procedure_actual_arg(call_arg, arg_src, false);
                if (i < n_args - 1) args += ", ";
                continue;
            }
            ASR::Variable_t *param = param_sym != nullptr
                && ASR::is_a<ASR::Variable_t>(*param_sym)
                ? ASR::down_cast<ASR::Variable_t>(param_sym) : nullptr;
            ...
            bool implicit_external_array_section_actual = is_c
                ...
                && ASRUtils::is_array(param_type);
            std::string section_arg;
            if (implicit_external_array_section_actual
                    && try_get_fortran_external_array_section_actual(
                        shape_call_arg, section_arg)) {
                args += section_arg;
                if (i < n_args - 1) args += ", ";
                continue;
            ...
            if (is_c && param_defines_actual) {
                ...
                if (actual_var != nullptr && param_type_unwrapped != nullptr
                        && type_unwrapped != nullptr
                        && ASRUtils::is_character(*param_type_unwrapped)
                        && ASRUtils::is_character(*type_unwrapped)
                        && !ASRUtils::is_array(param_type)
                        && !ASRUtils::is_array(type)) {
                    ASR::String_t *str_type = ASRUtils::get_string_type(call_arg);
                    if (str_type != nullptr && str_type->m_len != nullptr) {
                        std::string len_setup;
                        std::string actual_len = get_string_length_expr(call_arg, arg_src, len_setup);
                        src = arg_src;
                        std::string target = get_c_mutable_scalar_expr(call_arg);
                        std::string call_indent(indentation_level * indentation_spaces, ' ');
                        call_arg_setup += len_setup;
                        call_arg_setup += call_indent + "if (" + target
                            + " == NULL || ((int64_t)_lfortran_str_len(" + target
                            + ")) < ((int64_t)(" + actual_len + "))) " + target
                            + " = (char*) _lfortran_string_malloc_alloc(_lfortran_get_default_allocator(), "
                            + actual_len + ");\n";
                    }
                }
            ...
            if (shape_call_arg && ASR::is_a<ASR::Var_t>(*shape_call_arg)
                ...
                if (is_c && param != nullptr
                        && (param->m_value_attr
                            || (f_type != nullptr && f_type->m_abi == ASR::abiType::BindC))
                        && param_type_unwrapped != nullptr
                        && type_unwrapped != nullptr
                        && ASR::is_a<ASR::CPtr_t>(*param_type_unwrapped)
                    ...
                    if (is_c_unspecified_scalar_ref_dummy(param)
                            && !can_take_fortran_external_scalar_actual_address(
                                call_arg, shape_call_arg)) {
                        args += get_c_scalar_ref_actual(param, call_arg,
                            shape_call_arg, src);
                    } else {
                        args += address_of_src(src);
                    }
                ...
                if (wants_aggregate_pointer_actual) {
                    ...
                    if (raw_pointer_actual) {
                        args += src;
                    } else if (is_c_unspecified_scalar_ref_dummy(param)
                            && !can_take_fortran_external_scalar_actual_address(
                                call_arg, shape_call_arg)) {
                        args += get_c_scalar_ref_actual(param, call_arg,
                            shape_call_arg, src);
    ...
    bool is_c_string_concat_callee(const ASR::Function_t *f,
            const std::string &sym_name) const {
        if (!is_c || f == nullptr) {
            return false;
        }
        std::string fn_name = std::string(f->m_name);
        return fn_name.find("_lcompilers_stringconcat") != std::string::npos
            || sym_name.find("_lcompilers_stringconcat") != std::string::npos;
    }

    bool is_c_string_trim_callee(const ASR::Function_t *f,
            const std::string &sym_name) const {
        if (!is_c || f == nullptr) {
            return false;
        }
        std::string fn_name = std::string(f->m_name);
        return fn_name.find("_lcompilers_trim") != std::string::npos
            || sym_name.find("_lcompilers_trim") != std::string::npos;
    }

    bool is_c_string_len_trim_callee(const ASR::Function_t *f,
            const std::string &sym_name) const {
        if (!is_c || f == nullptr) {
            return false;
        }
        std::string fn_name = std::string(f->m_name);
        return fn_name.find("_lcompilers_len_trim") != std::string::npos
            || sym_name.find("_lcompilers_len_trim") != std::string::npos;
    }

    bool try_emit_c_string_concat_subroutine_call(const ASR::SubroutineCall_t &x,
            ASR::Function_t *f, const std::string &sym_name, std::string &out) {
        if (!is_c_string_concat_callee(f, sym_name) || x.n_args != 5) {
            return false;
        }

        auto materialize_operand = [&](ASR::expr_t *expr, std::string &value,
                std::string &length, std::string &setup,
                std::string &cleanup) -> bool {
            if (expr == nullptr) {
                return false;
            }
            self().visit_expr(*expr);
            ...
            setup += drain_tmp_buffer();
            setup += extract_stmt_setup_from_expr(value);
            materialize_allocating_string_expr(expr, value, length, setup, cleanup);
            return true;
        };
        ...
        std::string left_len;
        std::string right;
        std::string right_len;
        if (!materialize_operand(x.m_args[0].m_value, left, left_len, setup, cleanup)
                || !materialize_operand(x.m_args[1].m_value, right, right_len,
                    setup, cleanup)) {
            return false;
        }

        ASR::expr_t *result_expr = x.m_args[4].m_value;
        ...
        std::string result = src;
        setup += drain_tmp_buffer();
        setup += extract_stmt_setup_from_expr(result);
        ASR::expr_t *raw_result_expr = unwrap_c_lvalue_expr(result_expr);
        ASR::expr_t *shape_result_expr =
            raw_result_expr ? raw_result_expr : result_expr;
        std::string result_lvalue =
    ...
    bool try_emit_c_string_trim_subroutine_call(const ASR::SubroutineCall_t &x,
            ASR::Function_t *f, const std::string &sym_name, std::string &out) {
        if (!is_c_string_trim_callee(f, sym_name) || x.n_args != 2) {
            return false;
        }

        ASR::expr_t *input_expr = x.m_args[0].m_value;
        ASR::expr_t *result_expr = x.m_args[1].m_value;
        if (input_expr == nullptr || result_expr == nullptr) {
            return false;
        }
        ...
        setup += extract_stmt_setup_from_expr(input);
        std::string cleanup;
        std::string input_len;
        materialize_allocating_string_expr(input_expr, input, input_len, setup, cleanup);
        ...
        std::string trim_len = get_unique_local_name("__lfortran_trim_len");
        out = setup
            + indent + "int64_t " + trim_len
            + " = _lfortran_str_len_trim(" + input + ", " + input_len + ");\n"
            + indent + "_lfortran_strcpy_alloc(_lfortran_get_default_allocator(), &"
    ...
    bool try_emit_c_string_len_trim_function_call(const ASR::FunctionCall_t &x,
            ASR::Function_t *f, const std::string &fn_name, std::string &out) {
        if (!is_c_string_len_trim_callee(f, fn_name) || x.n_args != 1
                || x.m_args[0].m_value == nullptr) {
            return false;
        }
        ...
        std::string setup = drain_tmp_buffer();
        setup += extract_stmt_setup_from_expr(input);
        std::string input_len = get_string_length_expr(input_expr, input, setup);
        if (!setup.empty()) {
            tmp_buffer_src.push_back(setup);
        }
        out = "_lfortran_str_len_trim(" + input + ", " + input_len + ")";
        return true;
    }
    ...
    void visit_FunctionCall(const ASR::FunctionCall_t &x) {
        ...
        if (fn != nullptr && sym_info[get_hash((ASR::asr_t*)fn)].intrinsic_function) {
            ...
            if (is_c) {
                ...
                if (try_emit_c_string_len_trim_function_call(x, fn, fn_name, inline_call)) {
                    src = check_tmp_buffer() + inline_call;
                    last_expr_precedence = 2;
                    return;
                }
            ...
            bool force_generated_call_abi = callee_is_procedure_variable;
            std::string call_args = construct_call_args(
                fn, x.n_args, x.m_args, force_generated_call_abi,
                true, false);
            if (is_c && fn_name == "strlen" && x.n_args == 1
                    && x.m_args[0].m_value != nullptr
                    && ASR::is_a<ASR::CPtr_t>(
                        *ASRUtils::type_get_past_allocatable_pointer(
                            ASRUtils::expr_type(x.m_args[0].m_value)))
                    && call_args.rfind("&", 0) == 0) {
                call_args = call_args.substr(1);
            }
    ...
    void visit_StringSection(const ASR::StringSection_t& x) {
        ...
        std::string setup = drain_tmp_buffer();
        setup += extract_stmt_setup_from_expr(arg);
        std::string arg_len = get_string_length_expr(x.m_arg, arg, setup);
    ...
    void visit_Assignment(const ASR::Assignment_t &x) {
        ...
        if (try_emit_c_self_section_realloc_assignment(
                unwrapped_target_expr, x.m_value)) {
            return;
        }
        if (try_emit_c_reshape_char_array_bitcast_to_numeric_array_assignment(
        ...
        if (try_emit_c_rank1_section_array_constant_assignment(
                x, unwrapped_target_expr)) {
            return;
        }
        ...
        if (try_emit_c_rank2_section_scalar_assignment(
                unwrapped_target_expr, x.m_value)) {
            return;
        }
        ...
        if (is_c && ASR::is_a<ASR::StringItem_t>(*x.m_target)) {
            ...
            std::string updated_value =
                ...
                idx + ", " + idx + ", 1, true, true)";
            std::string target_len =
                CUtils::get_fixed_character_length_arg(ASRUtils::expr_type(si->m_arg));
            bool target_has_fixed_len = target_len != "NULL";
            src += indent + "char* " + updated_value_name + " = " + updated_value + ";\n";
            src += indent + "_lfortran_strcpy_alloc(_lfortran_get_default_allocator(), &"
                + string_arg + ", " + target_len + ", true, "
                + (target_has_fixed_len ? "false" : "true") + ", " + updated_value_name
                + ", " + string_len + ");\n";
        ...
        if (is_c && ASR::is_a<ASR::StringSection_t>(*x.m_target)) {
            ...
            std::string target_len =
                CUtils::get_fixed_character_length_arg(ASRUtils::expr_type(ss->m_arg));
            bool target_has_fixed_len = target_len != "NULL";
            src += indent + "char* " + updated_value_name + " = " + updated_value + ";\n";
            src += indent + "_lfortran_strcpy_alloc(_lfortran_get_default_allocator(), &"
                + string_arg + ", " + target_len + ", true, "
    ...
    std::string arr_get_single_element(std::string array,
        ...
        if( data_only || is_fixed_size ) {
            ...
            if (is_unbounded_pointer_to_data) {
                ...
                std::string offset = cache ? cache->offset : array + "->offset";
                tmp = data + "[(" + offset + " + " + idx + ")]";
    ...
    void visit_Var(const ASR::Var_t &x) {
        ...
        if (is_c) {
            if (current_function
                    ...
                    && !ASRUtils::is_array(sv->m_type)) {
                src = "(*((void**)" + var_name + "))";
            } else if (sv->m_value_attr
                    && ASR::is_a<ASR::CPtr_t>(*ASRUtils::type_get_past_allocatable_pointer(sv->m_type))
                    && !ASRUtils::is_array(sv->m_type)) {
                src = var_name;
            ...
            if (owner && CUtils::is_symbol_owner<ASR::AssociateBlock_t>(owner)
                ...
                || sv->m_intent == ASRUtils::intent_out
                || is_c_unspecified_scalar_ref_dummy(sv))
    ...
    void visit_Cast(const ASR::Cast_t &x) {
        ...
        switch (x.m_kind) {
            ...
            case (ASR::cast_kindType::IntegerToComplex) : {
                if (is_c) {
                    headers.insert("complex.h");
                    src = get_c_complex_expr(x.m_type, src, "0");
                } else {
            ...
            case (ASR::cast_kindType::RealToComplex) : {
                if (is_c) {
                    headers.insert("complex.h");
                    src = get_c_complex_expr(x.m_type, src, "0.0");
                } else {
                    src = "std::complex<double>(" + src + ")";
                }
                last_expr_precedence = 2;
    ...
    std::string get_c_complex_expr(ASR::ttype_t *type,
            const std::string &re, const std::string &im) {
        int kind = ASRUtils::extract_kind_from_ttype_t(type);
        if (kind == 4) {
            return "((float_complex_t)((float)(" + re + ") + I * (float)("
                + im + ")))";
        }
        return "((double_complex_t)((double)(" + re + ") + I * (double)("
            + im + ")))";
    }
    ...
    void visit_ComplexConstructor(const ASR::ComplexConstructor_t &x) {
        ...
        src = get_c_complex_expr(x.m_type, re, im);
    }
    ...
    bool try_emit_c_integer_power(ASR::expr_t *left_expr, ASR::expr_t *right_expr,
            const std::string &left, const std::string &right) {
        if (!is_c) {
            return false;
        }
        ...
        int left_kind = ASRUtils::extract_kind_from_ttype_t(left_type);
        if (left_kind == 4) {
            src = "__lfortran_c_powi_i32(" + left + ", " + right + ")";
        } else if (left_kind == 8) {
            src = "__lfortran_c_powi_i64(" + left + ", " + right + ")";
        } else {
            return false;
        }
        last_expr_precedence = 2;
    ...
    void handle_BinOp(const T &x) {
        ...
        switch (x.m_op) {
            ...
            case (ASR::binopType::Pow) : {
                ...
                if (try_emit_c_integer_power(x.m_left, x.m_right, left, right)) {
                    return;
                }
        ...
        for (size_t i=0; i<x.n_args; i++) {
            ...
            if( ASR::is_a<ASR::Var_t>(*tmp_expr) ) {
                ...
                if (ASR::is_a<ASR::Variable_t>(*var_sym)) {
                    ...
                    } else if (is_c && is_scalar_allocatable_dummy_slot_type(var)) {
            ...
            if (ASRUtils::is_array(type)) {
                ...
                out += indent + sym + "->is_allocated = true;\n";
                if (needs_array_zero_init) {
                    std::string init_indent = indent;
                    if (!did_allocate_name.empty()) {
                        out += indent + "if (" + did_allocate_name + ") {\n";
                        init_indent += "    ";
                    }
                    out += init_indent + "if (" + sym + "->data != NULL) {\n";
                    std::string data_init_indent = init_indent + "    ";
                    out += data_init_indent + "memset(" + sym + "->data, 0, " + size_str + ");\n";
                    if (element_struct_t != nullptr) {
                        std::string init_idx = get_unique_local_name("__lfortran_struct_array_init_i");
                        out += data_init_indent + "for (int64_t " + init_idx + " = 0; " + init_idx
                            + " < " + elem_count + "; " + init_idx + "++) {\n";
                        std::string elem_ptr = "(&(" + sym + "->data[" + init_idx + "]))";
                        out += data_init_indent + "    " + elem_ptr + "->"
                        ...
                        self().initialize_struct_instance_members(
                            element_struct_t, out, data_init_indent + "    ", elem_ptr);
                        out += data_init_indent + "}\n";
                    }
                    out += init_indent + "}\n";
                    if (!did_allocate_name.empty()) {
                        out += indent + "}\n";
    ...
    void visit_Stop(const ASR::Stop_t &x) {
        ...
        if (x.m_code) {
            ...
            std::string stop_code = src;
            ASR::ttype_t *stop_type = ASRUtils::type_get_past_allocatable_pointer(
                ASRUtils::expr_type(x.m_code));
            if (stop_type != nullptr && ASRUtils::is_character(*stop_type)) {
                if (is_c) {
                    headers.insert("stdio.h");
                    src = indent + "fprintf(stderr, \"STOP %s\\n\", "
                        + stop_code + ");\n";
                } else {
                    src = indent + "std::cerr << \"STOP \" << " + stop_code
                        + " << std::endl;\n";
                }
                src += indent + "exit(0);\n";
                return;
    ...
    bool try_emit_c_rank1_reduction_induction_loop(const ASR::DoLoop_t &loop,
        ...
        if (!is_c || current_function == nullptr
                ...
                || !is_c_simple_loop_bound_expr(loop.m_head.m_start)
                || !is_c_simple_loop_bound_expr(loop.m_head.m_end)) {
            return false;
        }
        ASR::symbol_t *loop_sym = get_c_var_symbol_from_expr(loop.m_head.m_v);
        ASR::Assignment_t *assignment =
            ASR::down_cast<ASR::Assignment_t>(loop.m_body[0]);
        ASR::symbol_t *target_sym = get_c_var_symbol_from_expr(assignment->m_target);
        if (loop_sym == nullptr || target_sym == nullptr
                || !ASR::is_a<ASR::Variable_t>(*loop_sym)
        ...
        ASR::ArrayItem_t *item = ASR::down_cast<ASR::ArrayItem_t>(
            ASRUtils::get_past_array_physical_cast(right));
        if (item->n_args != 1
                || !is_c_same_var_expr(get_array_index_expr(item->m_args[0]),
                    loop_sym)) {
            return false;
        }
        ...
        const CArrayDescriptorCache *cache =
            get_current_function_array_descriptor_cache(array, 1);
        if (cache == nullptr || cache->data.empty()
                || cache->lower_bounds.empty() || cache->strides.empty()) {
            return fail_with_restore();
        }
        ...
        if (!loop_bound(loop.m_head.m_start, loop_start)
                || !loop_bound(loop.m_head.m_end, loop_end)) {
            return fail_with_restore();
        }

        std::string loop_name = CUtils::get_c_variable_name(
            *ASR::down_cast<ASR::Variable_t>(loop_sym));
    ...
    bool try_emit_c_rank3_reduction_induction_loop(const ASR::DoLoop_t &outer,
            std::string &out) {
        if (!is_c || current_function == nullptr
                || outer.n_body != 1
                || !ASR::is_a<ASR::DoLoop_t>(*outer.m_body[0])
        ...
        ASR::DoLoop_t *middle = ASR::down_cast<ASR::DoLoop_t>(outer.m_body[0]);
        if (middle->n_body != 1
                || !ASR::is_a<ASR::DoLoop_t>(*middle->m_body[0])
                || !is_c_unit_increment_loop(middle->m_head)
                || !is_c_simple_loop_bound_expr(middle->m_head.m_start)
                || !is_c_simple_loop_bound_expr(middle->m_head.m_end)) {
            return false;
        }
        ASR::DoLoop_t *inner = ASR::down_cast<ASR::DoLoop_t>(middle->m_body[0]);
        if (inner->n_body != 1 || !ASR::is_a<ASR::Assignment_t>(*inner->m_body[0])
                || !is_c_unit_increment_loop(inner->m_head)
                || !is_c_simple_loop_bound_expr(inner->m_head.m_start)
                || !is_c_simple_loop_bound_expr(inner->m_head.m_end)) {
        ...
        ASR::symbol_t *outer_sym = get_c_var_symbol_from_expr(outer.m_head.m_v);
        ASR::symbol_t *middle_sym = get_c_var_symbol_from_expr(middle->m_head.m_v);
        ASR::symbol_t *inner_sym = get_c_var_symbol_from_expr(inner->m_head.m_v);
        if (outer_sym == nullptr || middle_sym == nullptr || inner_sym == nullptr) {
            return false;
        }
        ASR::Assignment_t *assignment =
            ASR::down_cast<ASR::Assignment_t>(inner->m_body[0]);
        ...
        if (!ASR::is_a<ASR::Variable_t>(*outer_sym)
                || !ASR::is_a<ASR::Variable_t>(*middle_sym)
                || !ASR::is_a<ASR::Variable_t>(*inner_sym)
                || !ASR::is_a<ASR::Variable_t>(*target_sym)) {
            return false;
        }
        ...
        if (item->n_args != 3
                || !is_c_same_var_expr(get_array_index_expr(item->m_args[0]), inner_sym)
                || !is_c_same_var_expr(get_array_index_expr(item->m_args[1]), middle_sym)
                || !is_c_same_var_expr(get_array_index_expr(item->m_args[2]), outer_sym)) {
            return false;
        }
        ...
        const CArrayDescriptorCache *cache =
            get_current_function_array_descriptor_cache(array, 3);
        if (cache == nullptr || cache->data.empty()
                || cache->lower_bounds.size() < 3 || cache->strides.size() < 3) {
            return fail_with_restore();
        }
        ...
        std::string outer_start, outer_end, middle_start, middle_end;
        std::string inner_start, inner_end;
        if (!loop_bound(outer.m_head.m_start, outer_start)
                || !loop_bound(outer.m_head.m_end, outer_end)
                || !loop_bound(middle->m_head.m_start, middle_start)
                || !loop_bound(middle->m_head.m_end, middle_end)
                || !loop_bound(inner->m_head.m_start, inner_start)
                || !loop_bound(inner->m_head.m_end, inner_end)) {
            return fail_with_restore();
        }
        ...
        std::string outer_name = CUtils::get_c_variable_name(
            *ASR::down_cast<ASR::Variable_t>(outer_sym));
        std::string middle_name = CUtils::get_c_variable_name(
            *ASR::down_cast<ASR::Variable_t>(middle_sym));
        std::string inner_name = CUtils::get_c_variable_name(
            *ASR::down_cast<ASR::Variable_t>(inner_sym));
        std::string target_name = CUtils::get_c_variable_name(
            *ASR::down_cast<ASR::Variable_t>(target_sym));
        ...
        std::string ptr_name = get_unique_local_name(ptr_prefix);
        std::string indent(indentation_level * indentation_spaces, ' ');
        std::string middle_indent((indentation_level + 1) * indentation_spaces, ' ');
        std::string inner_indent((indentation_level + 2) * indentation_spaces, ' ');
        std::string body_indent((indentation_level + 3) * indentation_spaces, ' ');
        out = indent + "for (" + outer_name + "=" + outer_start + "; "
            + outer_name + "<=" + outer_end + "; " + outer_name + "++) {\n";
        out += middle_indent + "for (" + middle_name + "=" + middle_start
            + "; " + middle_name + "<=" + middle_end + "; " + middle_name
            + "++) {\n";
        out += inner_indent + element_type + " * " + ptr_name
            + " = " + cache->data + " + " + cache->offset + " + ("
            + cache->strides[2] + " * (" + outer_name + " - "
            + cache->lower_bounds[2] + ")) + (" + cache->strides[1]
            + " * (" + middle_name + " - " + cache->lower_bounds[1]
            + ")) + (" + cache->strides[0] + " * (" + inner_start
            + " - " + cache->lower_bounds[0] + "));\n";
        out += inner_indent + "for (" + inner_name + "=" + inner_start + "; "
            + inner_name + "<=" + inner_end + "; " + inner_name + "++) {\n";
        out += body_indent + target_name + " = " + target_name + " "
    ...
    void visit_DoLoop(const ASR::DoLoop_t &x) {
        ...
        if (try_emit_c_rank3_reduction_induction_loop(x, optimized_loop)) {
            src = optimized_loop;
            return;
        }
        ...
        if (ASR::is_a<ASR::Var_t>(*x.m_head.m_v)) {
            loop_sym = ASRUtils::symbol_get_past_external(
                ASR::down_cast<ASR::Var_t>(x.m_head.m_v)->m_v);
        }
        std::string lvname;
        if (loop_sym != nullptr && ASR::is_a<ASR::Variable_t>(*loop_sym)) {
            ASR::Variable_t *loop_var = ASR::down_cast<ASR::Variable_t>(loop_sym);
            if (expects_c_scalar_ref_actual(loop_var)) {
                self().visit_expr(*x.m_head.m_v);
                lvname = src;
            } else {
                lvname = CUtils::get_c_variable_name(*loop_var);
            }
        } else {
            self().visit_expr(*x.m_head.m_v);
            lvname = src;
    ...
    void visit_SubroutineCall(const ASR::SubroutineCall_t &x) {
        ...
        if (is_c) {
            std::string string_concat_call;
            if (try_emit_c_string_concat_subroutine_call(
                    x, s, sym_name, string_concat_call)) {
                src = string_concat_call;
                return;
            }
            std::string string_trim_call;
    ...
    void visit_BitCast(const ASR::BitCast_t &x) {
        ...
        if (is_c && src_expr && target_type && source_type
                ...
                && (ASRUtils::is_integer(*source_type)
                    ...
                    || ASRUtils::is_real(*source_type)
                    || ASRUtils::is_logical(*source_type))
                && ASRUtils::get_type_byte_size(target_type)
                    == ASRUtils::get_type_byte_size(source_type)) {
            headers.insert("string.h");
            self().visit_expr(*src_expr);
            std::string source = src;
            std::string target_c_type = CUtils::get_c_type_from_ttype_t(target_type);
            std::string source_c_type = CUtils::get_c_type_from_ttype_t(source_type);
            std::string target_code = CUtils::get_c_type_code(target_type, true, false);
            std::string source_code = CUtils::get_c_type_code(source_type, true, false);
            src = c_utils_functions->get_bitcast_scalar_to_scalar(
                target_c_type, source_c_type, source_code, target_code)
                + "(" + source + ")";
            return;
        }
        if (x.m_value) {
